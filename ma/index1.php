...
//						 FROM peinvestments AS pe, industry AS i, pecompanies AS pec,stage as s
                        ...
                        pec.countryId,c.country, pe.AcquirerId,ac.Acquirer,ac.countryid,pe.Asset,pe.hideamount,DATE_FORMAT( DealDate, '%M-%Y' ) as dates,DealDate as DealDate FROM mama AS pe, industry AS i,
                       ...
                       $ordertype="asc";
                        ...
                        $companysqlFinal = "SELECT pe.PECompanyId, pec.companyname, pec.industry, i.industry, sector_business as sector_business, pe.Amount, pe.MAMAId, i.industry,
                        pec.countryId,c.country, pe.AcquirerId,ac.Acquirer,AggHide,ac.countryid,pe.Asset,pe.hideamount,DATE_FORMAT( DealDate, '%M-%Y' ) as dates,DealDate as DealDate FROM mama AS pe, industry AS i,
...
//                                                $sector_sql[] = " sector_business LIKE '$word%' ";
			...
			elseif($advisorsearchstring_legal!=" " && $advisorsearchstring_legal!="")
			{
                            ...
                            cia.CIAId,cia.Cianame,adac.CIAId AS AcqCIAId,peinv.AcquirerId, ac.Acquirer,peinv.Asset,peinv.hideamount,AggHide,DATE_FORMAT( DealDate, '%M-%Y' ) as dates,DealDate as DealDate
                            ...
                            cia.CIAId,cia.Cianame,adcomp.CIAId AS CompCIAId,peinv.AcquirerId, ac.Acquirer,peinv.Asset,peinv.hideamount,AggHide,DATE_FORMAT( DealDate, '%M-%Y' ) as dates,DealDate as DealDate
			...
			elseif (($industry > 0) || ($target_comptype!="--") || ($acquirer_comptype!= "--") || ($startRangeValue == "--") || ($endRangeValue == "--") || (($month1 != "--") && (year1 != "--")  && ($month2 !="--") && ($year2 != "--")))
				{
					...
					$companysql = "SELECT pe.PECompanyId, pec.companyname, pec.industry, i.industry,
							...
							pe.AcquirerId,ac.Acquirer,ac.countryid,pe.Asset,pe.hideamount,AggHide,DATE_FORMAT( DealDate, '%M-%Y' ) as dates,DealDate as DealDate
...
//                                $cl_count = count($_POST);
    ...
    <li><a id="icon-detailed-view" class="postlink" href="madealdetails.php?value=<?php echo $comid;?>" ><i></i> Detail View</a></li> 
			...
			$totalAmount=$totalAmount;
                ...
                if($studentOption==1)
                {
                 ?>
                         <script type="text/javascript" >
                                ...
                                $("#show-total-amount").html('<h2> Announced Value US$ <?php
                                if($totalAmount >0)
                               {
                               ...
                               {
                                   echo "--";
                               }?> M  across <?php echo $acrossDealsCnt; ?> deals;&nbsp;</h2>');
                            </script>
                ...
                $_SESSION['aggvalue']='<h2> Announced Value US$ '.$totalAmount.' M  across '.$acrossDealsCnt.' deals;&nbsp</h2>';
		}
		else
		{
                    if($exportToExcel==1)
                    ...
                    <?php
                    $_SESSION['aggvalue']='<h2> Announced Value US$ '.$totalAmount.' M  across '.$acrossDealsCnt.'deals;</h2>';
                    }
                    else
                    {
                    ?>
...
<?php } ?>
			...
			<input type="hidden" name="txthidesearchallfield" value=<?php echo $searchallfieldhidden; ?> >
                 ...
                 <?php } else { ?> 
                        ...
                        <input type="hidden" name="txtsearchon" value="4" >
                        <!--input type="hidden" name="txthidename" value=Team >
			<input type="hidden" name="txthideemail" value=team@kutung.com -->
			<input type="hidden" name="txthidename" value=<?php echo $username; ?> >
			<input type="hidden" name="txthideemail" value=<?php echo $UserEmail; ?> >
