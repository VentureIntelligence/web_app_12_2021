<?php

class deals extends database {
	
	var $elements;
	var $pkName;
	var $dbName;

	function deals() {
		
		database::database();	
		
		$this->elements["Ident"] 		= "";
		$this->elements["Heading"] 	= "";
		$this->elements["Description"] 	= "";
		$this->elements["Features"] 	= "";
		$this->elements["Image_Name"]    = "";
		$this->elements["OrginalPrice"]    = "";
		$this->elements["Value"] 	= "";
		$this->elements["Discount"] 	= "";
		$this->elements["You_Save"] 	= "";
		$this->elements["PayPal_BtnCode"] 	= "";
		$this->elements["Paypal_BtnDisplay"] = "";
		$this->elements["Meta_Title"]    = "";
		$this->elements["Meta_Desc"]     = "";
		$this->elements["BidCount"]     = "";
		$this->elements["Added_Date"] 	= "";

		$this->pkName="Ident";
		$this->dbName="deals";

	}
	
	function update($values) {

		$this->elements=$values;

		$this->elements[$this->pkName] = (!strlen($this->elements[$this->pkName]) || $this->elements[$this->pkName]=="") ? -1 : $this->elements[$this->pkName];	
		if ($this->elements[$this->pkName]!=-1) 
			$sql = $this->getUpdateSQL();
		else 
			$sql = $this->getInsertSQL();

		$this->execute($sql);

		if($this->elements[$this->pkName] == -1) {//added by rajaraman
			$this->execute('SELECT LAST_INSERT_ID() as id');
			$this->setFetchMode('ASSOC');
			$rs = $this->fetch();	
			$this->elements[$this->pkName] = $rs['id'];
		}

		if(!($this->error))
			return true;		
	}

	function getFullList($pageID=1,$rows=300,$fields=array("Ident","Name"),$where="",$order="",$type="num"){

		if(!strlen($pageID)) $pageID=1;
		if(!strlen($rows)) $rows=20;
		if(!strlen($fields)) $fields=array("Ident","Name");
		if(!strlen($where)) $where="";
		if(!strlen($order)) $order="";
	
		if($fields != '*')
			$fields = implode(",", $fields);

		$sql = "SELECT ".$fields." FROM ".$this->dbName." ";
		
		if(strlen($where)) $sql.= " WHERE ".$where;
		if(strlen($order))   $sql.= " ORDER BY ".$order;
		$sql.= " LIMIT ".(($pageID-1)*$rows).",".($rows);

		if($type=="name")
			$this->setFetchMode('ASSOC');
		
		$this->execute($sql);

		$return_array = array();
		$cont=0;
		if($fields=="*")
			$return_array=$this->fetch();
		else{
			while ($rs = $this->fetch()) {
				$return_array[$cont]=$rs;
				$cont++;
			}
		}
		return $return_array;
	}

	function count($where=""){
			
		$sql = "select count(".$this->pkName.") as total from ".$this->dbName;
		
		if(strlen($where))  $sql.= " WHERE ".$where;
		
		$this->setFetchMode('ASSOC');
		$this->execute($sql);
		$rs = $this->fetch();
		return $rs["total"]; 
	}	

	function select($ID){
		if(strlen($ID)){
			$sql = "select * from ".$this->dbName." where ".$this->pkName."=$ID";
			$this->setFetchMode('ASSOC');
			$this->execute($sql);
			$this->elements = $this->fetch();
		}
	}
	


	function delete($ID){
	
		if(strlen($ID)){
			
			$sql = "delete from ".$this->dbName." where ".$this->pkName."=$ID";
			$rs = $this->execute($sql);
			
		}
	}
	

	function getContactusList(){
		$sql = "select * from ".$this->dbName;
		$this->setFetchMode('ASSOC');
		$this->execute($sql);

		$return_array = array();
		$cont=0;
		if($fields=="*")
			$return_array=$this->fetch();
		else{
			while ($rs = $this->fetch()) {
				$return_array[$cont]=$rs;
				$cont++;
			}
		}
		return $return_array;
	}


	function GetImageName($imageFileName) {
		$imageFileName = basename($imageFileName);
		$imageFileName = str_replace(" ","",$imageFileName);
		$imageFileName = str_replace("'","",$imageFileName);
		$imageFileName = explode(".",$imageFileName);
		$md = md5($imageFileName[0]);
		$Enc = time().$md;
		$Expsplit = ".".$imageFileName[1];
		$ImageName = $Enc.$Expsplit;
		return $ImageName;
	}


	function getDealsPath($Key,$imageFileName) {
			$strDynamicPath = FOLDER_CREATE_PATH.'deals/';
			$strDynamicPath = $strDynamicPath.$Key."/".$imageFileName;
			return $strDynamicPath;
	}



	function getDeals(){
		$order = "Ident Desc";
	 	$sql = "select * from ".$this->dbName;
		if(strlen($order))   $sql.= " ORDER BY ".$order;
		$this->execute($sql);
		$return_array = array();
		while ($rs = $this->fetch()) {
			$return_array[$rs[0]]= $rs[1];
			$cont++;
		}
		return $return_array;
	}	


}//End of Class
?>