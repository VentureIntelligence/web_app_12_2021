...
elseif ($_REQUEST['sortby']=="sortdetailed") {
   $order.="FIELD(a.FY,'14') ".$_REQUEST['sortorder'].", FY ".$_REQUEST['sortorder'];
}
...
if($_REQUEST['searchv']!=''){
          if($where!=''){
                  $where .= " and b.SCompanyName like '".$_REQUEST['searchv']."%'";
          }else{
                  $where .= " b.SCompanyName like '".$_REQUEST['searchv']."%'";
          }
       }
	...
	if($_REQUEST['answer']['Region'] != ""){
		...
		//echo "1"; pr($where);
                 if($chargewhere!=''){
                    ...
                    $SearchResults = $plstandard->SearchHome_WithCharges($chargewhere,$fields,$where,$order,$group,"name",$page,$limit,$client='');
...
//echo "6";
      ...
      if($chargewhere!=''){
          $allgrowthResults = $growthpercentage->SearchHome_WithCharges_cnt($chargewhere,$fields,$where,$order,$group);
           
         // print_r($allgrowthResults);
         
           $total=$allgrowthResults[0]['count'];
         //$SearchExport = $plstandard->SearchHomeExport($fields,$where,$order,$group);
          $SearchExport2 = $growthpercentage->SearchHome_WithChargesExport($chargewhere,$fields,$where,$order,$group);
          $growthResults = $growthpercentage->SearchHome_WithCharges($chargewhere,$fields,$where,$order,$group,"name",$page,$limit,$client='');        
...
$where .= " and  a.CId_FK = b.Company_Id"; // Original Where
$group = " b.SCompanyName ";
$fields = array("a.CAGR_Id, a.CId_FK, a.IndustryId_FK,a.OptnlIncome,a.EBITDA,a.EBDT ,a.EBT,a.Tax,a.PAT ,max(a.FY) as FY, a.ResultType,b.Company_Id, b.FCompanyName,b.ListingStatus","a.TotalIncome as TotalIncome","b.Permissions1","b.FYCount"," b.SCompanyName"," b.Industry"," b.Sector");
...
$cagrResults = $cagr->SearchHome($fields,$where,$order,$group,$page,"name",$limit,$client='');
//pr($SearchResults);
