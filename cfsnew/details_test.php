...
$fields1 = array("*");
$where1 = "a.CID_FK = ".$_GET['vcid']." AND YEAR(`Added_Date`)=MAX(YEAR(`Added_Date`))";
$order1 = "a.FY DESC";
...
//pr($FinanceAnnual1); exit;
//$FinanceAnnual["GroupStausName"] .= $GroupCStaus[$FinanceAnnual['GroupCStaus']];
//pr($FinanceAnnual1);
$template->assign("FinanceAnnual1",$FinanceAnnual1);
$template->assign("FinanceAnnual2",count($FinanceAnnual1));

$template->assign("FinanceAnnual1",$FinanceAnnual1_new);
$template->assign("FinanceAnnual2",count($FinanceAnnual1_new));
...
if(file_exists(FOLDER_CREATE_PATH.'balancesheet_new/New_BalSheet_'.$_GET['vcid'].'.xls')){
	$template->assign("BSHEET_MEDIA_PATH_NEW",FOLDER_CREATE_PATH.'balancesheet_new/New_BalSheet_'.$_GET['vcid'].'.xls');
}
...
$company=trim($cprofile->elements['FCompanyName']);
...
if($valCount > 0)
{
	...
	echo " <br/>fileNameExt-"; echo $paragraph = $fileNameExt;

	if (preg_match_all($string, $paragraph, $matches)) {
	  echo " <br/>count match"; echo count($matches[0]);  // Total size
print_r($matches);
	echo " <br/>dateFileName-"; echo $dateFileName = $matches[0][ count($matches[0]) -1 ];	// get match from Right
...
//echo "<br><br>" ;
 ...
 $indexofcharge =   mysql_query("SELECT * FROM fd2_index_of_charges  WHERE CIN ='$cin' ORDER BY `Charge Holder`, `Date of Charge` ASC " );

 $indexofchargeitems ="";
while ($ioc = mysql_fetch_array($indexofcharge))
{
   $indexofchargeitems.="<tr>"; 
   $indexofchargeitems.="<td>".$ioc["Charge ID"]."</td>" ;
   $indexofchargeitems.="<td>".$ioc["Date of Charge"]."</td>" ;
   $indexofchargeitems.="<td>".$ioc["Charge amount secured"]."</td>" ;
   $indexofchargeitems.="<td>".$ioc["Charge Holder"]."</td>" ;
   $indexofchargeitems.="<td>".$ioc["Address"]."</td>" ;
   //$indexofchargeitems.="<td>".$ioc["SRN"]."</td>" ;
   $indexofchargeitems.="</tr>";
//$indexofchargeitems.= "<tr><td>$ioc[chargeid]</td><td>$ioc[dateofcharge]</td></tr>" ;
}
...
$con = mysql_connect("localhost","venture_cpslogin","Cps$2010") or die(mysql_error());

mysql_select_db("venture_cps", $con);
...
$con1 = mysql_connect("localhost","venture_cpslogin","Cps$2010",true) or die(mysql_error());
	...
	mysql_select_db("venture_cps", $con1);
        ...
        //echo "<br>SELECT * FROM cin_detail where $where limit 0,1";
...
$template->assign("indexofchargeitems",$indexofchargeitems);
