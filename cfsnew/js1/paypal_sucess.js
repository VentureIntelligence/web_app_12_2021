function InsertPayeeInformation() {
	var flag		=		0;

	var Deals  = $('Deals').value;
	var Name  = $('Name').value;
	var MailId= $('EmailId').value;
	var MobNum= $('MobNum').value;
	var Address = $('Address').value;
	var security_code = $('security_code').value;


	if(Deals ==""){
		$('CatErrorMsg').innerHTML = "Please Select a Deal !";
		$('Deals').focus();
		flag=1;	
	}else if(Name ==""){
		$('CatErrorMsg').innerHTML = "Please Enter Name !";
		$('Name').focus();
		flag=1;	
	}else if(MailId ==""){
		$('CatErrorMsg').innerHTML = "Please Enter Email Id !";
		$('EmailId').focus();
		flag=1;	
	}else if(!isValidEmail(MailId)){
		$('CatErrorMsg').innerHTML = "Please Enter valid Email Id !";
		$('EmailId').focus();
		flag=1;	
	}else if(MobNum ==""){
		$('CatErrorMsg').innerHTML = "Please Enter Mobile Number!";
		$('MobNum').focus();
		flag=1;	
	}else if(!isValidPhone(MobNum)){
		$('CatErrorMsg').innerHTML = "Please Enter Valid Mobile Number !";
		$('MobNum').focus();
		flag=1;	
	}else if(Address ==""){
		$('CatErrorMsg').innerHTML = "Please Enter the Address !";
		$('Address').focus();
		flag=1;	
	}else if(security_code ==""){
		$('CatErrorMsg').innerHTML = "Please Enter Security Code !";
		$('security_code').focus();
		flag=1;	
	}
