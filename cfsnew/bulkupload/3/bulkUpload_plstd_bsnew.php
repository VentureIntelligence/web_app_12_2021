...
//if(isset($_GET["action"]) && ($_GET["action"] == "upload")){
    ...
    /*if( file_exists($b_outfile) ) {*/
        ...
        $subject = "File started - plstd bsnew cagr ";
        //$txt = "Started successfully !!! @ $start";
        ...
        while (($plstandard = fgetcsv($file, 10000, ",")) !== FALSE) {
         $count++;
            if($count>1){
           // if($count>7499 && $count < 14999){
           // if($count>14999 && $count < 22499){
           // if($count>22499 && $count < 30000){
                if($plstandard[21] !='' && $plstandard[21] !='CIN Number'){
                    ...
                    if(mysql_num_rows($result) >0){
                        ...
                        $EBTbeforePeriod = $plstandard[22];
                        $priorPeriod = $plstandard[23];
                        ...
                        if($pl_check_count > 0){
                            ...
                            mysql_query("update plstandard set IndustryId_FK='".$res['Industry']."',OptnlIncome='$OptnlIncome',OtherIncome='$OtherIncome',OptnlAdminandOthrExp='$OptnlAdminandOthrExp',OptnlProfit='$OptnlProfit',EBITDA='$EBITDA',Interest='$Interest',EBDT='$EBDT',Depreciation='$Depreciation',EBT='$EBT',Tax='$Tax',PAT='$PAT',BINR='$BINR',DINR='$DINR',TotalIncome='$TotalIncome',EmployeeRelatedExpenses='$EmployeeRelatedExpenses',ForeignExchangeEarningandOutgo='$ForeignExchangeEarningandOutgo',EarninginForeignExchange='$EarninginForeignExchange',OutgoinForeignExchange='$OutgoinForeignExchange',AddFinancials='$AddFinancials',updated_date='$added_date',EBT_before_Priod_period='$EBTbeforePeriod',Priod_period='$priorPeriod' where PLStandard_Id='$pls_id'");
                            mysql_query("insert into test_cin (cin,file_name,action,added_date,pl_bs_id,source_flag) values ('$cin','plstd','update','$added_date','$pls_id','$source_flag')");
                        }else{
                            $sql_insert_pl = "INSERT INTO plstandard 
                                            (
                                                ...
                                                ,OutgoinForeignExchange,AddFinancials,updated_date,EBT_before_Priod_period,Priod_period   
                                            ) 
                                            VALUES 
                                            (
                                                ...
                                                ,'$EmployeeRelatedExpenses','$ForeignExchangeEarningandOutgo','$EarninginForeignExchange','$OutgoinForeignExchange','$AddFinancials','$added_date','$EBTbeforePeriod','$priorPeriod'
                                            )";
                            $result_insert_pl = mysql_query($sql_insert_pl) or die(mysql_error()); 
                            mysql_query("insert into test_cin (cin,file_name,action,added_date,pl_bs_id,source_flag) values ('$cin','plstd','insert','$added_date','$result_insert_pl','$source_flag')"); 
                        }
                        mysql_query("UPDATE cprofile SET source_flag2='$source_flag' WHERE Company_id='$cprofile_id'");
                    }else{
                        mysql_query("insert into test_cin (cin,file_name,action,added_date,source_flag) values ('$cin','plstd','no_company','$added_date','$source_flag')");
                    }
                }
            }
        }        //die;*/
        ...
        while (($balancenew = fgetcsv($file, 10000, ",")) !== FALSE) {
           ...
           // if($count>7000 && $count < 14000){
          //  if($count>14000 && $count < 21000){
            if($count>1){
                if($balancenew[38] !=''){
                    $cin = $balancenew[38];
                    $result = mysql_query("SELECT Company_Id,Industry FROM cprofile WHERE CIN='$cin'");
                    if(mysql_num_rows($result) >0){
                        ...
                        if($bs_check_count > 0){
                            ...
                            mysql_query("insert into test_cin (cin,file_name,action,added_date,pl_bs_id,source_flag) values ('$cin','bs','update','$added_date','$bs_id','$source_flag')");
                        }else{
                            ...
                            mysql_query("insert into test_cin (cin,file_name,action,added_date,pl_bs_id,source_flag) values ('$cin','bs','insert','$added_date','$result_insert_bs','$source_flag')");
                        }
                        mysql_query("UPDATE cprofile SET source_flag2='$source_flag' WHERE Company_id='$cprofile_id'");
                    }else{
                        mysql_query("insert into test_cin (cin,file_name,action,added_date,source_flag) values ('$cin','bs','no_company','$added_date','$source_flag')");
                    }
                }
            }
        } //die;*/
        ...
        //$cprofile_check = mysql_query("SELECT cin FROM test_cin WHERE source_flag='$source_flag' group by cin limit 3900,700" );
        $result_type = 0;
        //$cprofile_check = mysql_query("SELECT cin FROM test_cin WHERE source_flag='$source_flag' group by cin limit $start,700" );
        $cprofile_check = mysql_query("SELECT cin FROM test_cin WHERE source_flag='$source_flag' group by cin" );
        $cin_check_count = mysql_num_rows($cprofile_check);
        if($cin_check_count > 0){
            $u = 0;
            while($cprofileDetails = mysql_fetch_array($cprofile_check)){
                $cin_number = $cprofileDetails['cin'];
                
                $cprofileD = mysql_query("SELECT * FROM cprofile WHERE CIN='$cin_number'" );
                if(mysql_num_rows($cprofileD) > 0){
                    $cprofileDs = mysql_fetch_array( $cprofileD );
                    $cprofile_id = $cprofileDs['Company_Id'];
                    $IndustryId_FK = $cprofileDs['Industry'];
                    $pl_details = mysql_query("SELECT * FROM plstandard WHERE CId_FK='$cprofile_id' AND ResultType = " . $result_type . " order by FY desc" );
        ...
        mysql_query("insert into test (start_id,added_date,total_cnt) values ('$start','$added_date','$u')");
