...
#mobileApi_length select{
   width: 65px;
}
#mobileApi_length label{
   font-size: 15px;
}
#mobileApi_length{
    float: right;
}
#userLists label ,#apiLists label{
	margin : 0px 15px;
}
#userLists select:focus ,#apiLists select:focus{
	outline: 0;
}
#userLists ,#apiLists{
	float: left;
}
#userLists select ,#apiLists select {
...
{/literal} 
 ...
 {literal} 
	$( document ).ready( function() {
		...
		var table ;	
		$(document).on("change",".filterType .filtered",function(){
			var filterList = $(this).val();
			if(filterList == 'Users') {
				if($('#mobileApi tr').length > 1) {
					table.clear().destroy();
					$("#userLists select ,#apiLists select ").remove();
				}
		...
		function recreate_table() {
			...
			initComplete: function () {
				this.api().columns('0,2').every(function () {
					...
					if (column.index() == 2) {
						var select = $('<select><option value="none">Please Select Users</option></select>')
						...
						var select = $('<select><option value="none">Please Select API Name</option></select>')
										.appendTo('.api_wrapper')
										.on('change', function () {
							var val = $.fn.dataTable.util.escapeRegex(
								$(this).val()
...
<form name="Frm_AddRating" id="Frm_AddRating" action="xbrlparse_ajx.php" method="post" enctype="multipart/form-data">
	...
	<div class="filterType">
		<span>Filter By :</span> 
		<input type="radio" name="filterBy" class="filtered" value="Users"><span class="radiobtn">Users</span>
        <input type="radio" name="filterBy" class="filtered" value="APIName"><span class="radiobtn">API Name</span>
	</div>
		<div class="search_box">
			...
			<div class="" id="userLists">
				<label id="req_answer">Users:</label>
				<div class="user_wrapper">
				</div>
				<div style="clear: both;"></div>
			</div>
			
			<div class="" id="apiLists" style="display:none">
				<label id="req_answer">API Name:</label>
			...
			<span style="position: relative;top: 7px; left: 10px;"><a href="{$ADMIN_BASE_URL}mobile_api_tracker_vi.php">Clear Search</a></span>
			<div style="clear: both;"></div>
		</div>
		...
		<label id="apiUrl" style=" font: bold 13px &quot;Courier New&quot;, Courier, monospace;  margin: 10px 10px; color: #000;  position: absolute;display:none;">API URL : http://162.214.29.33:3000/</label>
