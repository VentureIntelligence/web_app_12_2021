...
<script type="text/javascript">
    ...
    window.onload = function () { 
        ...
        $.get("ajax_mca_profile.php", {cin: ""+cin+""}, function(data){
            if(data == ''){
                var value = '';
            }else{
                var value = data;
        ...
        $.get("ajax_mca_director.php", {cin: ""+cin+""}, function(data){
            if(data == ''){
                var value = '';
            }else{
                var value = data;
            }
            $('#mca_data1').html(value);
            var row = $('#activeSubmenu').val();
            if(row !=''){
                tabMenu(row);
            }
        ...
        $.get("ajax_mca_charges.php", {cin: ""+cin+""}, function(data){
            if(data == ''){
                var value = '';
            }else{
                var value = data;
            }
            $('#mca_data2').html(value);
            var row = $('#activeSubmenu').val();
            $('#chargesRegistered').addClass('tab_menu');
            if(row !=''){
                tabMenu(row);
            }
            /*if(row !=''){
...
$(document).mouseup(function (e)
{
 ...
 if( ( !$(e.target).is("#check1") ) && ( $('#check1').attr('data-check1') == 'open' ) ) {
        $("#balancesheet_ex").hide();  
        $('#check1').attr("data-check1", "close");
     }*/
    ...
    //$('#check').attr("data-check", "close");
      ...
      function tabMenu(row){
        $('.tab_menu').hide();
        $('#'+row).show();
        if((row =='profit-loss') || (row == 'balancesheet') || (row == 'ratio')){
            $('.finance-filter').show();        
        }else{        
            $('.finance-filter').hide();
        }
...
<div class="lb" id="popup-boxDetail">
	<div class="title">Update Financials</div>
        <form name="addDBFinacials" id="addDBFinacials">
        	...
        	<label> From</label>
                <input type="text" name="u_fromaddress" id="u_fromaddress" value=""  />
        </div>
...
<div class="list-tab" style="clear: both;margin-top:15px;">
    <ul>
...
<div align="center">
    <div class="btn-cnt p10" style="{if $file_pl_cnt eq 2 || $file_bs_cnt eq 2 } margin-bottom: 42px {elseif $file_pl_cnt eq 1 || $file_bs_cnt eq 1} margin-bottom:  20px {else} margin-bottom:  0px {/if} ">
           {if $PLSTANDARD_MEDIA_PATH || $PLSTANDARD_MEDIA_PATH_CON}	
              <span style="  position: relative;  width: 175px;"> 
              <input  name="" type="button" id="check" data-check="open" value="P&L EXPORT" onClick="openpl_ex(this)" style=" background: #a37635 url(images/arrow-dropdown.png) no-repeat 158px 6px; width:180px; " />

          <div id="pl_ex" data-slide="close" style=" position: absolute;  width: 100%; {if $file_pl_cnt > 0 && $file_bs_cnt > 0} left:-1px{else} left:-2px {/if}">
       ...
       {if $BSHEET_MEDIA_PATH_NEW || $BSHEET_MEDIA_PATH_NEW1}	
              <span style="  position: relative;  width: 175px;"> 
              <input  name="" type="button" id="check1" data-check1="open" value="BALANCE SHEET EXPORT" onClick="openbalancesheet_ex(this)" style=" background: #a37635 url(images/arrow-dropdown.png) no-repeat 163px 6px; width:180px; " />

              <div id="balancesheet_ex" style="position: absolute;  width: 100%;  left:-2px;">
           ...
           {/if}
           <input class="postlink" type="button" onclick='location.href = "projectionall.php?vcid={$Company_Id}";' value="SEE PROJECTION" name="senddeal">
...
<tbody>  
 ...
 {/if}
  ...
  <div class="finance-cnt postContainer postContent masonry-container">
   ...
   <div  class="work-masonry-thumb col-4 tab_menu" id="companyProfile" href="http://erikjohanssonphoto.com/work/aishti-ss13/">
   ...
   <tr>
       ...
       {if $CompanyProfile.Website neq ""}
            <td> Website
                <span><a href="{$CompanyProfile.Website_url}" target="_blank">{$CompanyProfile.Website}</a></span>
            </td> 
       {else}
           <td> Website
       ...
       <td></td>
       <td></td>
        {if $CompanyProfile.auditor_name neq ""}
            <td> Auditor Name
            <span>{$CompanyProfile.auditor_name}</span> </td>
       {/if}
    </tr> 
       {if $CompanyProfile.BusinessDesc neq ""} 
    <tr>
        <td> Business Description
                <span>{$CompanyProfile.BusinessDesc}</span>
            </td> 
       <td></td>
       <td></td>
       <td></td>
    ...
    {if $companylinkedIn neq ""}
     <tr id="viewlinkedin_loginbtn">
         ...
         <td colspan="5"> View LinkedIn Profile 
...
{literal}
   ...
   <span id="mca_data1" class="tab_menu"><div style="margin-bottom:20px;">MCA data look up... <img src="images/loadingAnimation.gif" style="margin-left:10px;"/></div></span>
       <div  class="work-masonry-thumb col-4 tab_menu" href="http://erikjohanssonphoto.com/work/aishti-ss13/" id="rating-info">
    ...
    <tr>
     ...
     <tr>
        ...
        <form action="http://www.smera.in/live_ratings.php" name="smera_form" id="smera_form" method="post" target='_blank'>
   ...
   <span id="mca_data2" class="tab_menu"><div style="margin-bottom:20px;">MCA data look up... <img src="images/loadingAnimation.gif" style="margin-left:10px;"/></div></span>
     {if $industry2}                                              
    <div id="similar-company"  class="work-masonry-thumb col-4 tab_menu" href="http://erikjohanssonphoto.com/work/aishti-ss13/">
...
<div class="click-top">
    <a href="javascript:;"><img src="images/arrow-up-.png" title="Back to Top" alt="Back to Top" class="backtotop"/></a>
</div>
            {literal}
<script>
