...
function millconversion(str,vcid){
	...
	$.get("ajaxmillicurrencyreal.php", {queryString: ""+ccur1+"",vcid:""+vcid+"",rconv:""+str+"",yoy:""+yoyr+""}, function(data){
			$('#result').html(data);
			$('.displaycmp').hide();
                        $('#stMsg').hide();
			resetfoundation();
...
{$roc}
      ...
      {/if}  
                ...
                {if $PLSTANDARD_MEDIA_PATH}
                    <div class="btn-cnt p10">
                       <input  name="" type="button" value="Standard P&L EXPORT" onClick="window.location.href='downloadtrack.php?vcid={$Company_Id}'" /></div>
		...
		{if $PLDETAILED_MEDIA_PATH}
                    <div class="btn-cnt p10">
                    <input  name="" type="button" value="Detailed P&L EXPORT" onClick="window.location.href='{$MEDIA_PATH}pldetailed/PLDetailed_{$VCID}.xls'" /></div>
		...
		{if $BSHEET_MEDIA_PATH}	
                    <div class="btn-cnt p10">
                    <form> <input  name="" type="button" value="BALANCESHEET EXPORT" onClick="window.location.href='{$MEDIA_PATH}balancesheet/BalSheet_{$VCID}.xls'" /></form></div>
		{/if}
		{if $CASHFLOW_MEDIA_PATH}
                    <div class="btn-cnt p10">
                        <input  name="" type="button" value="CASHFLOW EXPORT" onClick="window.location.href='{$MEDIA_PATH}cashflow/Cashflow_{$VCID}.xls'" /></div>
			<!--div>&nbsp;</div><div class="anchorblue">To download CashFlow :&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a  href="{$MEDIA_PATH}cashflow/Cashflow_{$VCID}.xls">Click Here</a></div-->
		{/if}
		{else}
			<h1>"Your Subscription limit of {$grouplimit[0][2]} companies has been reached. Please contact info@ventureintelligence.com to top up your subscription"</h1><br/>
                    {/if}
