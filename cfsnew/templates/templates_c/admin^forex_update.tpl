<?php /* Smarty version 2.5.0, created on 2018-08-03 12:48:59
         compiled from admin/forex_update.tpl */ ?>
...
<script>
	...
	$j(document).ready(function(){
		$j( \'#btn-log-submit\' ).click( function() {
			...
			$j.ajax({
				...
				success: function(response){
		           	...
		           	if( respData.FY.length != 0 ) {
		           		respData.FY.each(function(value,key){
		           			var outgoContent = \'\';
		           			var earningContent = \'\';
		           			var outgoAttr = \'\';
		           			var earningAttr = \'\';
		           			if( value.OutgoinForeignExchange > 0 ) {
		           				outgoContent = \'<span class="edit_outgoearning">Edit</span>\';
		           				outgoAttr = \'readonly\';
		           			}
		           			if( value.EarninginForeignExchange > 0 ) {
		           				earningContent = \'<span class="edit_outgoearning">Edit</span>\';
		           				earningAttr = \'readonly\';
		           			}
		           			content += \'<tr><td>\'+value.FY+\'<input type="hidden" name="fy[]" value="\'+value.FY+\'" /></td><td class="forex_in_box"><input type="text" name="outgoForeignExchange[]" class="outgoForeignExchange" value="\'+value.OutgoinForeignExchange+\'" \'+outgoAttr+\' />\'+outgoContent+\'</td><td class="forex_in_box"><input type="text" name="earningForeignExchange[]" class="earningForeignExchange" value="\'+value.EarninginForeignExchange+\'" \'+earningAttr+\' />\'+earningContent+\'</td></tr>\';
						});
		           	} else {
		...
		$j( \'#btn-forex-submit\' ).click(function() {
			...
			if( $j( \'.outgoForeignExchange[value=""]\' ).length > 0 ) {
				alert( \'please enter outgo foreign exchange value\' );
				checkFlag = false;
				return false;	
			} else if( $j( \'.earningForeignExchange[value=""]\' ).length > 0 ) {
				alert( \'please enter earning foreign exchange value\' );
				checkFlag = false;
				return false;	
			}

			$j(\'input[name="outgoForeignExchange[]"]\').each(function() { 
				var aValue = $j(this).val();
				if( isNaN(aValue) ) {
					alert( \'Only integer allowed in outgo foreign exchange\' );
					checkFlag = false;
					return false;	
				}
			});
			$j(\'input[name="earningForeignExchange[]"]\').each(function() { 
				var bValue = $j(this).val();
				if( isNaN(bValue) ) {
					alert( \'Only integer allowed in earning foreign exchange\' );
					checkFlag = false;
					return false;	
				}
			});
			if( checkFlag ) {
				var r = confirm("Do you wan to update the Forex value!");
				if ( r == true ) {
					$j( \'.body-overlay \' ).show();
				   $j.ajax({
						type: \'POST\',
						url: \'forexupdateval_ajx.php\',
						data: form.serialize(),
						success: function(response){
							$j( \'.body-overlay \' ).hide();
				           alert( \'Forex updated successfully\' );
				           location.reload();
				        }
					}); 
				} else {
...
index.php">Back to Home</a></span></div>
		<div class="adtitle" align="center">FOREX UPDATE</div>
