<?php /* Smarty version 2.5.0, created on 2020-02-21 11:31:50
         compiled from admin/mobile_pe_api_details.tpl */ ?>
...
index.php">CFS Admin Panel</a>
               ...
               <ul class="nav">
                  <?php if ($this->_tpl_vars['Usr_Flag'] == 1 || $this->_tpl_vars['Usr_Flag'] == 2): ?><li <?php if ($this->_tpl_vars['curPage'] == 'pmanagement.php'): ?>class="active"<?php endif; ?>><a href="<?php echo @constant('ADMIN_BASE_URL'); ?>
pmanagement.php">Company Profile</a></li><?php endif; ?>
                  <?php if ($this->_tpl_vars['Usr_Flag'] == 1 || $this->_tpl_vars['Usr_Flag'] == 2): ?><li <?php if ($this->_tpl_vars['curPage'] == 'fmanagement.php'): ?>class="active"<?php endif; ?>><a href="<?php echo @constant('ADMIN_BASE_URL'); ?>
...
<script type="text/javascript">
   var mobileapi = <?php echo $this->_tpl_vars['apitrackinglist']; ?>
;
   var apicount=<?php echo $this->_tpl_vars['apicountlist']; ?>
;
   <?php echo ' 
         ...
         $(document).ready(function(){
              ...
              for(i=0 ; i<mobileapi.length;i++) {
                  mobileApiList.push(mobileapi[i].user);
                       ...
                       //$(\'#optionList\').val(\'arun@ventureintelligence.in\');
              }
          });
          $(document).ready(function(){
              ApiList = [] ;
              for(i=0 ; i<apicount.length;i++) {
                  ApiList.push(apicount[i].apiName);
                       $("#apiList").append(\'<option value="\'+apicount[i].apiName+\'">\'+apicount[i].apiName+\'</option>\');
                       $(\'#apiList\').val(\'/login\');
              }
          });
         $(document).on("click","#submit-data",function(){
                  $("#totalCount,#apiurlname,#apiurlcount,.apiname-value").empty();
                  userList();
         });
         $(document).ready(function(e){
            $("#search_data").on(\'submit\', function(e){
               ...
               $("#drp").val(drp);
                $(".apiname-value").text(\'\');
               $("#totalCount,#apiurlcount").text(\'\');
               $(\'#apiurlname\').text(\'No data for this API\');
               var apiname = $("#apiList").val();
               e.preventDefault();
               $.ajax({
                  type: \'POST\',
                  url: "pe_ajaxmobiletracking.php",
                  ...
                  success: function(response){
                     ...
                     if(response !=\'\'){
                        $("#totalCount,#apiurlname,#apiurlcount,#totalCount,#betweendateCount,.apiname-value").empty();
                        $(".search_count_datas").show();
                        var counlist = JSON.parse(response);
                        //$(".apiname-value").append(apiname);
                        var a=counlist.apinamevalue;
                        var apivalue1=a.replace(/\\//g, " - ");
                        var apivalue = apivalue1.substring(3, apivalue1.length);
                        $(".apiname-value").append(apivalue);
                        //$(".apiname-value").append(counlist.apinamevalue)
                        $("#apiurlname").append(counlist.apiurl);
                        $("#apiurlcount").append(counlist.search);
   ...
   <div class="container">
      <div class="content">
        <div class="loading" style="display: none;">
         ...
         <div>
            <span style="float:left; font-size: 13px; text-decoration: underline;"><a href="<?php echo @constant('ADMIN_BASE_URL'); ?>
index.php">Back to Home</a></span>
         </div>
         ...
         <div class="api-details">
            <form method="post" id="search_data" name="search_data" enctype="multipart/form-data" data-toogle="validator">
               <div class="widget-selectUser">
                  ...
                  <div class="col-md-6" style="width:80%;">
                     <fieldset class="fieldset-user" style="float: left;margin-right: 5px;">
                        <legend>Select API Name :</legend>
                        <select name="apiname" id="apiList" required>
                           <option hidden>Select API</option>
                        </select>
...
">
                  ...
                  <div class="api-countlist">
                     <fieldset class="fieldset-user " style="width:100%;" >
                        <legend>API:</legend>
                        ...
                        <tbody>
                        <tr class="apivaluelist">
                        <td class="apiname-value"></td>
                        <td id="apiurlname">No data for this api</td>
                        <td id="apiurlcount"></td>
                        </tr></tbody>
                        </table> 
            ...
            var drp = $(\'#reportrange span\').text();
             $(".apiname-value").text(\'\');
               ...
               $.ajax({ 
                      type: \'POST\',
                      url: "pe_ajaxmobiletracking.php",
                      data: {"username": username,"apiname":apiname, "drp": drp},
                     success: function(response){
                        showLoader();
                        console.log(response);
                        if(response !=\'\'){
                           ...
                           var counlist = JSON.parse(response);
                           $("#totalCount").append(counlist.totalcount);
                           $("#profileCount").append(counlist.search);
                           $("#financialCount").append(counlist.companylist);*/
                            $("#totalCount,#apiurlname,#apiurlcount,#totalCount,#betweendateCount,.apiname-value").empty();
                        $(".search_count_datas").show();
                        var counlist = JSON.parse(response);
                        var a=counlist.apinamevalue;
