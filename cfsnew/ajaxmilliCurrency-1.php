...
$fields = array("PLStandard_Id","CId_FK","IndustryId_FK","OptnlIncome","OtherIncome","OptnlAdminandOthrExp","OptnlProfit","EBITDA","Interest","EBDT","Depreciation","EBT","Tax","PAT","FY","TotalIncome","BINR","DINR","EmployeeRelatedExpenses","ForeignExchangeEarningandOutgo","EarninginForeignExchange","OutgoinForeignExchange","EBT_before_Priod_period","Priod_period");
...
if(count($FinanceAnnual1_new)==0){
    $FinanceAnnual1_new = $balancesheet_new->getFullList_withoutPL(1,100,$fields1,$where_withoutPL,$order1,"name"); 
} 

$where1 = "a.CID_FK = ".$_GET['vcid']." and a.ResultType='0' and b.ResultType=0";
...
<?php 
   // echo count($FinanceAnnual);echo count($FinanceAnnual1);echo count($FinanceAnnual1_new);
    if((count($FinanceAnnual) > 0) || (count($FinanceAnnual1) > 0) || (count($FinanceAnnual1_new) > 0)){
    ?>
    ...
    <div class="tab-res" style="overflow-x: auto; margin:10px 0 !important;">
        <table width="100%" border="0" cellspacing="0" cellpadding="0">
            ...
            ?>
             ...
             <?php for($i=0;$i<count($FinanceAnnual);$i++){ 
                if($_GET['queryString']!='INR'){?>
                    <td><?php if($FinanceAnnual[$i][TotalIncome]==0){echo '-';}else{$vale = currency_convert($FinanceAnnual[$i][TotalIncome],'INR',$_GET['queryString']);$tot=$vale/$convalue;echo round($tot,2); if($vale==''){echo '-';}} ?></td>
                      <?php
                }
                else
                {
            ...
            ?>
              ...
              <tr>
                ...
                <?php for($i=0;$i<count($FinanceAnnual);$i++){ 
                if($_GET['queryString']!='INR'){?>
                    <td><?php if($FinanceAnnual[$i][EBITDA]==0){echo '-';}else{ $vale = currency_convert($FinanceAnnual[$i][EBITDA],'INR',$_GET['queryString']);$tot=$vale/$convalue;echo round($tot,2);  if($vale==''){echo '-';}} ?></td>
                      <?php
                }
                else
                {
            ...
            ?>
              ...
              <tr>
               ...
               <?php
                ...
                {
                    ?>    
                      <td><?php  echo "<div style='display:none'>test 2</div>"; if($FinanceAnnual[$i][EBT_before_Priod_period] == 0 && $FinanceAnnual[$i][Priod_period] == 0){ $tot= ($FinanceAnnual[$i][EBT]/$convalue);echo round($tot,2);  }else if($FinanceAnnual[$i][EBT_before_Priod_period]==0){echo '-';}else{ $tot= ($FinanceAnnual[$i][EBT_before_Priod_period]/$convalue);echo round($tot,2); } ?></td>
             <?php
                }
            }
            ?>
            ...
            ?>
              ...
              <tr>
                ...
                <?php for($i=0;$i<count($FinanceAnnual);$i++){ 
                if($_GET['queryString']!='INR'){?>
                    <td><?php if($FinanceAnnual[$i][PAT]==0){echo '-';}else{ $vale = currency_convert($FinanceAnnual[$i][PAT],'INR',$_GET['queryString']); $tot=$vale/$convalue;echo round($tot,2); if($vale==''){echo '-';}} ?></td>
                 <?php
                }
                else
                {
    ...
    ?>
     <?php  if(file_exists(FOLDER_CREATE_PATH.'balancesheet_new/New_BalSheet_'.$_GET['vcid'].'.xls')){
        ...
        if ($BSHEET_MEDIA_PATH_NEW || $BSHEET_MEDIA_PATH_NEW1) {?>
                ...
                <span class="btn-cnt" style="  position: relative; float:right;padding-top:0px !important;"> 
