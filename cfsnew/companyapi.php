                    ...
                    if($cprofile_id != ''){
                        ...
                        if(mysql_num_rows($PLStsQry) > 0){
                            while($res = mysql_fetch_array($PLStsQry)){
                                ...
                                $res1['OptnlIncome'] = ( $curreny_code == 'INR' ) ? trim( $res['OptnlIncome'] ) : currency_convert( trim( $res['OptnlIncome'] ), 'INR', $curreny_code );
                                $res1['OtherIncome'] = ( $curreny_code == 'INR' ) ? trim( $res['OtherIncome'] ) : currency_convert( trim( $res['OtherIncome'] ), 'INR', $curreny_code );
                                $res1['OptnlAdminandOthrExp'] = ( $curreny_code == 'INR' ) ? trim( $res['OptnlAdminandOthrExp'] ) : currency_convert( trim( $res['OptnlAdminandOthrExp'] ), 'INR', $curreny_code );
                                $res1['OptnlProfit'] = ( $curreny_code == 'INR' ) ? trim( $res['OptnlProfit'] ) : currency_convert( trim( $res['OptnlProfit'] ), 'INR', $curreny_code );
                                $res1['EBITDA'] = ( $curreny_code == 'INR' ) ? trim( $res['EBITDA'] ) : currency_convert( trim( $res['EBITDA'] ), 'INR', $curreny_code );
                                $res1['Interest'] = ( $curreny_code == 'INR' ) ? trim( $res['Interest'] ) : currency_convert( trim( $res['Interest'] ), 'INR', $curreny_code );
                                $res1['EBDT'] = ( $curreny_code == 'INR' ) ? trim( $res['EBDT'] ) : currency_convert( trim( $res['EBDT'] ), 'INR', $curreny_code );
                                $res1['Depreciation'] = ( $curreny_code == 'INR' ) ? trim( $res['Depreciation'] ) : currency_convert( trim( $res['Depreciation'] ), 'INR', $curreny_code );
                                $res1['EBT'] = ( $curreny_code == 'INR' ) ? trim( $res['EBT'] ) : currency_convert( trim( $res['EBT'] ), 'INR', $curreny_code );
                                $res1['Tax'] = ( $curreny_code == 'INR' ) ? trim( $res['Tax'] ) : currency_convert( trim( $res['Tax'] ), 'INR', $curreny_code );
                                $res1['PAT'] = ( $curreny_code == 'INR' ) ? trim( $res['PAT'] ) : currency_convert( trim( $res['PAT'] ), 'INR', $curreny_code );
                                $res1['FY'] = trim( $res['FY'] );
                                $res1['TotalIncome'] = ( $curreny_code == 'INR' ) ? trim( $res['TotalIncome'] ) : currency_convert( trim( $res['TotalIncome'] ), 'INR', $curreny_code );
                                $res1['BINR'] = ( $curreny_code == 'INR' ) ? trim( $res['BINR'] ) : currency_convert( trim( $res['BINR'] ), 'INR', $curreny_code );
                                $res1['DINR'] = ( $curreny_code == 'INR' ) ? trim( $res['DINR'] ) : currency_convert( trim( $res['DINR'] ), 'INR', $curreny_code );
                                ...
                                if($res['EmployeeRelatedExpenses']!= ""){
                                    $EmployeeRelatedExpenses1 = ( $curreny_code == 'INR' ) ? trim( $res['EmployeeRelatedExpenses'] ) : currency_convert( trim( $res['EmployeeRelatedExpenses'] ), 'INR', $curreny_code );
                                }else{
                                ...
                                if($res['EarninginForeignExchange']!= ""){
                                    $EarninginForeignExchange1 = ( $curreny_code == 'INR' ) ? trim( $res['EarninginForeignExchange'] ) : currency_convert( trim( $res['EarninginForeignExchange'] ), 'INR', $curreny_code );
                                }else{
                                ...
                                if($res['OutgoinForeignExchange']!= ""){
                                    $OutgoinForeignExchange1 = ( $curreny_code == 'INR' ) ? trim( $res['OutgoinForeignExchange'] ) : currency_convert( trim( $res['OutgoinForeignExchange'] ), 'INR', $curreny_code );
                                }else{
                        ...
                        $growth = array();
                            ...
                            if(mysql_num_rows($Qry) > 0){
                                while($res = mysql_fetch_array($Qry)){
                                    $res1 = array();
                                    $res1['FY'] = trim($res['FY']);
                                    $res1['ShareCapital'] = ( $curreny_code == 'INR' ) ? trim($res['ShareCapital']) : currency_convert( trim( $res['ShareCapital'] ), 'INR', $curreny_code );
                                    $res1['ShareApplication'] = ( $curreny_code == 'INR' ) ? trim($res['ShareApplication']) : currency_convert( trim( $res['ShareApplication'] ), 'INR', $curreny_code );
                                    $res1['ReservesSurplus'] = ( $curreny_code == 'INR' ) ? trim($res['ReservesSurplus']) : currency_convert( trim( $res['ReservesSurplus'] ), 'INR', $curreny_code );
                                    $res1['TotalFunds'] = ( $curreny_code == 'INR' ) ? trim($res['TotalFunds']) : currency_convert( trim( $res['TotalFunds'] ), 'INR', $curreny_code );
                                    $res1['SecuredLoans'] = ( $curreny_code == 'INR' ) ? trim($res['SecuredLoans']) : currency_convert( trim( $res['SecuredLoans'] ), 'INR', $curreny_code );
                                    $res1['UnSecuredLoans'] = ( $curreny_code == 'INR' ) ? trim($res['UnSecuredLoans']) : currency_convert( trim( $res['UnSecuredLoans'] ), 'INR', $curreny_code );
                                    $res1['LoanFunds'] = ( $curreny_code == 'INR' ) ? trim($res['LoanFunds']) : currency_convert( trim( $res['LoanFunds'] ), 'INR', $curreny_code );
                                    $res1['OtherLiabilities'] = ( $curreny_code == 'INR' ) ? trim($res['OtherLiabilities']) : currency_convert( trim( $res['OtherLiabilities'] ), 'INR', $curreny_code );
                                    $res1['DeferredTax'] = ( $curreny_code == 'INR' ) ? trim($res['DeferredTax']) : currency_convert( trim( $res['DeferredTax'] ), 'INR', $curreny_code );
                                    $res1['SourcesOfFunds'] = ( $curreny_code == 'INR' ) ? trim($res['SourcesOfFunds']) : currency_convert( trim( $res['SourcesOfFunds'] ), 'INR', $curreny_code );
                                    $res1['GrossBlock'] = ( $curreny_code == 'INR' ) ? trim($res['GrossBlock']) : currency_convert( trim( $res['GrossBlock'] ), 'INR', $curreny_code );
                                    $res1['LessAccumulated'] = ( $curreny_code == 'INR' ) ? trim($res['LessAccumulated']) : currency_convert( trim( $res['LessAccumulated'] ), 'INR', $curreny_code );
                                    $res1['NetBlock'] = ( $curreny_code == 'INR' ) ? trim($res['NetBlock']) : currency_convert( trim( $res['NetBlock'] ), 'INR', $curreny_code );
                                    $res1['CapitalWork'] = ( $curreny_code == 'INR' ) ? trim($res['CapitalWork']) : currency_convert( trim( $res['CapitalWork'] ), 'INR', $curreny_code );
                                    $res1['FixedAssets'] = ( $curreny_code == 'INR' ) ? trim($res['FixedAssets']) : currency_convert( trim( $res['FixedAssets'] ), 'INR', $curreny_code );
                                    $res1['IntangibleAssets'] = ( $curreny_code == 'INR' ) ? trim($res['IntangibleAssets']) : currency_convert( trim( $res['IntangibleAssets'] ), 'INR', $curreny_code );
                                    $res1['OtherNonCurrent'] = ( $curreny_code == 'INR' ) ? trim($res['OtherNonCurrent']) : currency_convert( trim( $res['OtherNonCurrent'] ), 'INR', $curreny_code );
                                    $res1['Investments'] = ( $curreny_code == 'INR' ) ? trim($res['Investments']) : currency_convert( trim( $res['Investments'] ), 'INR', $curreny_code );
                                    $res1['DeferredTaxAssets'] = ( $curreny_code == 'INR' ) ? trim($res['DeferredTaxAssets']) : currency_convert( trim( $res['DeferredTaxAssets'] ), 'INR', $curreny_code );
                                    $res1['SundryDebtors'] = ( $curreny_code == 'INR' ) ? trim($res['SundryDebtors']) : currency_convert( trim( $res['SundryDebtors'] ), 'INR', $curreny_code );
                                    $res1['CashBankBalances'] = ( $curreny_code == 'INR' ) ? trim($res['CashBankBalances']) : currency_convert( trim( $res['CashBankBalances'] ), 'INR', $curreny_code );
                                    $res1['Inventories'] = ( $curreny_code == 'INR' ) ? trim($res['Inventories']) : currency_convert( trim( $res['Inventories'] ), 'INR', $curreny_code );
                                    $res1['LoansAdvances'] = ( $curreny_code == 'INR' ) ? trim($res['LoansAdvances']) : currency_convert( trim( $res['LoansAdvances'] ), 'INR', $curreny_code );
                                    $res1['OtherCurrentAssets'] = ( $curreny_code == 'INR' ) ? trim($res['OtherCurrentAssets']) : currency_convert( trim( $res['OtherCurrentAssets'] ), 'INR', $curreny_code );
                                    $res1['CurrentAssets'] = ( $curreny_code == 'INR' ) ? trim($res['CurrentAssets']) : currency_convert( trim( $res['CurrentAssets'] ), 'INR', $curreny_code );
                                    $res1['Provisions'] = ( $curreny_code == 'INR' ) ? trim($res['Provisions']) : currency_convert( trim( $res['Provisions'] ), 'INR', $curreny_code );
                                    $res1['CurrentLiabilitiesProvision'] = ( $curreny_code == 'INR' ) ? trim($res['CurrentLiabilitiesProvision']) : currency_convert( trim( $res['CurrentLiabilitiesProvision'] ), 'INR', $curreny_code );
                                    $res1['NetCurrentAssets'] = ( $curreny_code == 'INR' ) ? trim($res['NetCurrentAssets']) : currency_convert( trim( $res['NetCurrentAssets'] ), 'INR', $curreny_code );
                                    $res1['ProfitLoss'] = ( $curreny_code == 'INR' ) ? trim($res['ProfitLoss']) : currency_convert( trim( $res['ProfitLoss'] ), 'INR', $curreny_code );
                                    $res1['Miscellaneous'] = ( $curreny_code == 'INR' ) ? trim($res['Miscellaneous']) : currency_convert( trim( $res['Miscellaneous'] ), 'INR', $curreny_code );
                                    $res1['TotalAssets'] = ( $curreny_code == 'INR' ) ? trim($res['TotalAssets']) : currency_convert( trim( $res['TotalAssets'] ), 'INR', $curreny_code );
                            ...
                            if(mysql_num_rows($Qry) > 0){
                                while($res = mysql_fetch_array($Qry)){
                                    $res1 = array();
                                    $res1['FY'] = $res['FY'];
                                    $res1['ShareCapital'] = ( $curreny_code == 'INR' ) ? $res['ShareCapital'] : currency_convert( trim( $res['ShareCapital'] ), 'INR', $curreny_code );
                                    $res1['ReservesSurplus'] = ( $curreny_code == 'INR' ) ? $res['ReservesSurplus'] : currency_convert( trim( $res['ReservesSurplus'] ), 'INR', $curreny_code );
                                    $res1['TotalFunds'] = ( $curreny_code == 'INR' ) ? $res['TotalFunds'] : currency_convert( trim( $res['TotalFunds'] ), 'INR', $curreny_code );
                                    $res1['ShareApplication'] = ( $curreny_code == 'INR' ) ? $res['ShareApplication'] : currency_convert( trim( $res['ShareApplication'] ), 'INR', $curreny_code );
                                    $res1['L_term_borrowings'] = ( $curreny_code == 'INR' ) ? $res['L_term_borrowings'] : currency_convert( trim( $res['L_term_borrowings'] ), 'INR', $curreny_code );
                                    $res1['deferred_tax_liabilities'] = ( $curreny_code == 'INR' ) ? $res['deferred_tax_liabilities'] : currency_convert( trim( $res['deferred_tax_liabilities'] ), 'INR', $curreny_code );
                                    $res1['O_long_term_liabilities'] = ( $curreny_code == 'INR' ) ? $res['O_long_term_liabilities'] : currency_convert( trim( $res['O_long_term_liabilities'] ), 'INR', $curreny_code );
                                    $res1['L_term_provisions'] = ( $curreny_code == 'INR' ) ? $res['L_term_provisions'] : currency_convert( trim( $res['L_term_provisions'] ), 'INR', $curreny_code ); 
                                    $res1['T_non_current_liabilities'] = ( $curreny_code == 'INR' ) ? $res['T_non_current_liabilities'] : currency_convert( trim( $res['T_non_current_liabilities'] ), 'INR', $curreny_code );
                                    $res1['S_term_borrowings'] = ( $curreny_code == 'INR' ) ? $res['S_term_borrowings'] : currency_convert( trim( $res['S_term_borrowings'] ), 'INR', $curreny_code );
                                    $res1['Trade_payables'] = ( $curreny_code == 'INR' ) ? $res['Trade_payables'] : currency_convert( trim( $res['Trade_payables'] ), 'INR', $curreny_code );
                                    $res1['O_current_liabilities'] = ( $curreny_code == 'INR' ) ? $res['O_current_liabilities'] : currency_convert( trim( $res['O_current_liabilities'] ), 'INR', $curreny_code );
                                    $res1['S_term_provisions'] = ( $curreny_code == 'INR' ) ? $res['S_term_provisions'] : currency_convert( trim( $res['S_term_provisions'] ), 'INR', $curreny_code );
                                    $res1['T_current_liabilities'] = ( $curreny_code == 'INR' ) ? $res['T_current_liabilities'] : currency_convert( trim( $res['T_current_liabilities'] ), 'INR', $curreny_code );
                                    $res1['T_equity_liabilities'] = ( $curreny_code == 'INR' ) ? $res['T_equity_liabilities'] : currency_convert( trim( $res['T_equity_liabilities'] ), 'INR', $curreny_code );
                                    $res1['Tangible_assets'] = ( $curreny_code == 'INR' ) ? $res['Tangible_assets'] : currency_convert( trim( $res['Tangible_assets'] ), 'INR', $curreny_code );     
                                    $res1['Intangible_assets'] = ( $curreny_code == 'INR' ) ? $res['Intangible_assets'] : currency_convert( trim( $res['Intangible_assets'] ), 'INR', $curreny_code );
                                    $res1['T_fixed_assets'] = ( $curreny_code == 'INR' ) ? $res['T_fixed_assets'] : currency_convert( trim( $res['T_fixed_assets'] ), 'INR', $curreny_code );
                                    $res1['N_current_investments'] = ( $curreny_code == 'INR' ) ? $res['N_current_investments'] : currency_convert( trim( $res['N_current_investments'] ), 'INR', $curreny_code );
                                    $res1['Deferred_tax_assets'] = ( $curreny_code == 'INR' ) ? $res['Deferred_tax_assets'] : currency_convert( trim( $res['Deferred_tax_assets'] ), 'INR', $curreny_code );
                                    $res1['L_term_loans_advances'] = ( $curreny_code == 'INR' ) ? $res['L_term_loans_advances'] : currency_convert( trim( $res['L_term_loans_advances'] ), 'INR', $curreny_code );
                                    $res1['O_non_current_assets'] = ( $curreny_code == 'INR' ) ? $res['O_non_current_assets'] : currency_convert( trim( $res['O_non_current_assets'] ), 'INR', $curreny_code );
                                    $res1['T_non_current_assets'] = ( $curreny_code == 'INR' ) ? $res['T_non_current_assets'] : currency_convert( trim( $res['T_non_current_assets'] ), 'INR', $curreny_code );
                                    $res1['Current_investments'] = ( $curreny_code == 'INR' ) ? $res['Current_investments'] : currency_convert( trim( $res['Current_investments'] ), 'INR', $curreny_code );

                                    $res1['Inventories'] = ( $curreny_code == 'INR' ) ? $res['Inventories'] : currency_convert( trim( $res['Inventories'] ), 'INR', $curreny_code );
                                    $res1['Trade_receivables'] = ( $curreny_code == 'INR' ) ? $res['Trade_receivables'] : currency_convert( trim( $res['Trade_receivables'] ), 'INR', $curreny_code );
                                    $res1['Cash_bank_balances'] = ( $curreny_code == 'INR' ) ? $res['Cash_bank_balances'] : currency_convert( trim( $res['Cash_bank_balances'] ), 'INR', $curreny_code );
                                    $res1['S_term_loans_advances'] = ( $curreny_code == 'INR' ) ? $res['S_term_loans_advances'] : currency_convert( trim( $res['S_term_loans_advances'] ), 'INR', $curreny_code );
                                    $res1['O_current_assets'] = ( $curreny_code == 'INR' ) ? $res['O_current_assets'] : currency_convert( trim( $res['O_current_assets'] ), 'INR', $curreny_code );
                                    $res1['T_current_assets'] = ( $curreny_code == 'INR' ) ? $res['T_current_assets'] : currency_convert( trim( $res['T_current_assets'] ), 'INR', $curreny_code );
                                    $res1['Total_assets'] = ( $curreny_code == 'INR' ) ? $res['Total_assets'] : currency_convert( trim( $res['Total_assets'] ), 'INR', $curreny_code );
