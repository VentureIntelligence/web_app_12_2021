...
//pr($Fields);
	//   $GroupList1 = $grouplist->getFullList($Page,$Row,$Fields1=array('*'),$where,$order,"name");
        ...
        $Fields = array("Group_Id","G_Name","VisitLimit","ExLimit","SubLimit","Used","ExCount","SubCount","Create_Date" ,"status","expiry_date", " (SELECT COUNT(GroupList) FROM users WHERE users.GroupList=grouplist.Group_Id) as countusers ", "api_access", "mobile_app_access","admin_api_access" );
	...
	if($ListCnt == 0){
       ...
       // $pages['link'] = 'http://'.DOMAIN.WEB_DIR.'admin/editGroup_API.php?'.$url.'page=';
        $pages['link'] = 'http://localhost/ventureintelligence/cfsnew/admin/editGroup_API.php?'.$url.'page=';
...
$template->display("admin/editGroup_API.tpl");
