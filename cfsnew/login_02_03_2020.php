        ...
        //if (count($groupIp) > 0){ //Process IP Restriction only if user has IP addeded to account
            ...
            if($authAdmin['user_authorized_status']==1){
              ...
              if($user_os=='IOS'){      
                ...
                $sqlChkdevice = "SELECT * FROM `user_authorized_device` WHERE `user_email`= '".$authAdmin['username']."' and `device_details`='".$device_details."' and `status`=1";
                ...
                $displayMessage = 'Login Error - Device not authorized. We have sent Authorization Code to your email address. <a href="<?php echo GLOBAL_BASE_URL; ?>cfsnew/auth.php">Click Here to Complete Authorization</a>';
...
function sendAuthEmail($poc,$groupId,$allowedDevices,$userEmail){
    ...
    $message 	.= "<p><b>Authorization Link:</b> <a href='<?php echo GLOBAL_BASE_URL; ?>cfsnew/auth.php'> <?php echo GLOBAL_BASE_URL; ?>cfsnew/auth.php</a></p>";
...
// $count=count($devices_array)-3;
// $arraydel=array_splice($devices_array, 0, $count);
// foreach ($arraydel as $item) {
//   $itemid=$item['id'];  
//  $checkdevices = "DELETE FROM `user_authorized_device` WHERE id='$itemid'";
//  mysql_query($checkdevices);
