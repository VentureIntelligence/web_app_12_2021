  ...
  */
  //setcookie("TCook");
				...
				if((trim($login)!= "") && (trim($pwd)!=""))
				{
					...
					$checkForUnknownLoginSql="select * from RElogin_members where EmailId='$login'";
        ...
        dealcompanies AS dc WHERE dm.DCompId = dc.DCompId AND dm.EmailId='$login' and dm.Passwrd='$pwd' AND dc.Deleted =0";
                    ...
                    $devicenotauthorized = 0;
                                                                    $currCookie = $_COOKIE['reLoginAuth'];
                        ...
                        //$currCookie = $_COOKIE['peLoginAuth'];
                            ...
                            $sqlChkdevice = "SELECT * FROM `user_authorized_device` WHERE `user_email`= '".$myrow['EmailId']."' and `device_details`='".$device_details."' and `status`=1";
                            ...
                            if($cntChkdevice > 0){
                                ...
                                $sqlChkdevices = "SELECT * FROM `user_authorized_device` WHERE `user_email`='".$myrow['EmailId']."'";
                      ...
                      /*  if($device_authorized){
                                                                        ...
                                                                        $currCookie = $_COOKIE['reLoginAuth'];
                                                                        $sqlChkCookie = "SELECT `deviceId` FROM `userlog_device` WHERE `deviceId`='".$currUserCookie."' AND `EmailId`='".$myrow['EmailId']."' AND `dbType`='RE'";
                                                                        ...
                                                                        if ($currUserCookie!='' && $cntChkCookie >= 1 && $myrow['Student']==0){
                                                                           ...
                                                                           // if ($test ==0){     //commend this line
                                                                                ...
                                                                                if ($devicesUsed < $myrow['deviceCount']){
                                                                                    ...
                                                                                    $sqlInsCookie .= " VALUES ('".$myrow['EmailId']."','".$cookieName."','".$myrow['DCompId']."','RE')";
                                                                                    mysql_query($sqlInsCookie) or die(mysql_error());
                                                                                    $processLoginFlag = 1;
                                                                                ...
                                                                                if ($myrow['Student']==0){
                                                                                    ...
                                                                                    $displayMessage = 'Login Error - Device not authorized. We have sent Authorization Code to your email address. <a href="' . BASE_URL . 're/auth.php">Click Here to Complete Authorization</a>';
                                ...
                                if( date('Y-m-d')<=$myrow["ExpiryDate"])
									...
									if( date('Y-m-d')<=$myrow["ExpiryDate"])
									{
										session_register("REUserNames");
										session_register("REUserEmail");
										session_register("RELastActivity");
										session_register("REIP");
										$usernames=trim($myrow["Name"]);
										$_SESSION['REUserNames']=$usernames;
										$_SESSION['REUserEmail']=$myrow["EmailId"];
                                        $_SESSION["type"] = 'R';
                                                                                $_SESSION['REDcompanyId']=$myrow["DCompId"];
										$_SESSION['RELastActivity']=time();
                                                                                $_SESSION['REIP']="R";
                                                                                $_SESSION['student']=$myrow["Student"];
                                                                                 $_SESSION['RESession_Id']=$sesID;
                                                                                 $_SESSION['RE_TrialLogin']=$myrow["TrialLogin"];
                                                                                 $_SESSION['RE_EXPIRES'] = time() + 86400;
										//$explodeLogin=explode('@',$login);
										//if($explodeLogin[1]!='ventureintelligence.com')
										//{
											$LogIntimeSql="insert into dealLog(LogSessionID,EmailId,LoggedIn,IpAdd,PE_MA) values('$sesID','$login','$logintime','$ipadd',2)";
											if($logrs=mysql_query($LogIntimeSql))
											{
                                                                                        ...
                                                                                        $sqlUserLogSel = "SELECT `id` FROM `user_log` WHERE `emailId`='".$myrow['EmailId']."' AND `dbTYpe`='RE'";
                                                                                        $resUserLogSel = mysql_query($sqlUserLogSel);
                                                                                        $cntUserLogSel = mysql_num_rows($resUserLogSel);
                                                                                        if ($cntUserLogSel==0){
                                                                                            //Insert
                                                                                            $sqlInsUserLog = "INSERT INTO `user_log` (`userId`,`emailId`,`deviceId`,`sessionId`,`dbTYpe`) VALUES ('0','".$login."','".$deviceId."','".$sesID."','RE')";
					...
					elseif($random_cnt==0)
					{
						$RandomLogSql="insert into dealLog(LogSessionID,EmailId,LoggedIn,IpAdd,UnAuthorized,PE_MA) values('$sesID','$login','$logintime','$ipadd',1,2)";
...
function sendAuthEmail($companyId,$userEmail,$allowedDevices){
    ...
    $sqlInsAuth = "INSERT INTO `user_auth_code` (`user_id`,`emailId`,`dbType`,`reqOn`,`authCode`,`expDate`,`status`) VALUES ('0','".$userEmail."','RE','".$today."','".$authcode."','".$nextDate."','Active')";
    ...
    $from 	= 'subscription@ventureintelligence.com';
    $subject 	= "Authorization Code for RE Database(".$userEmail.")"; // Subject of the email

    //Message
    $message 	= 'Authorization Code for RE Database. Please find the details below:';

    $message 	.= "<p><b>User Email :</b> ".$userEmail."</p>";
    $message 	.= "<p><b>Authorization Code:</b> ".$authcode."</p>";
    $message 	.= "<p><b>Authorization Link:</b> <a href='" . BASE_URL . "re/auth.php'>" . BASE_URL . "re/auth.php</a></p>";
...
function checkFields()
 {
  	if((document.relogin.emailid.value == "") || (document.relogin.emailpassword.value == ""))
    {
 	...
 	if(document.relogin.chkTerm.checked==false)
 	{
...
<form name="relogin" onSubmit="return checkFields();" method="post" action="relogin.php" >
...
<ul>
    ...
    <li><input type="submit" name="btnSubmit" value ="Login" class="fp"/> <a href="<?php echo BASE_URL; ?>forgetpwd.php?value=R" class="forgot-link">Forgot your password?</a> </li>
...
<h3>Real Estate Deal Database </h3>

<p>The Venture Intelligence <i> Private Equity in Real Estate Database </i> lists investments in India-based Real Estate companies
     	and projects by private equity firms. The database also includes financial investments by
     	strategic investors which function similar to PE firms and often co-invest with them.</p>

<p>Each deal listed in this database typically includes the name of the investee, its location,
		type, the investors involved, the amount and date. Wherever available,
		we also include information on the valuation and the advisors to the deal.</p>

<p>Apart from the deal-by-deal listing, the database also provides aggregate private equity in Real Estate
		statistics that includes data on deals that have been disclosed in confidence
		to Venture Intelligence. The database also features a listing of PE exits, profiles of PE firms and PE-backed Real Estate companies/projects.</p>
...
<span class="popup_close"><a href="javascript: void(0);">X</a></span>
  <div class="popup_content" id="popup_content">
      ...
      <form name="authform" method="post" action="re/authdevice.php" onSubmit="return checkpopup();">
      <p  class="pop-header">Your Login is already authorized in the following 3 Browsers*. <br> Please select one of them to deauthorize</p>
      <div class="pop-div">
      <?php 
            ...
            $count=count($device_array_items)-3;
           ...
           foreach($device_array as $device_array_item) { 
                if ($device_array_item['device_details'] == @$targer_device_compare) {	
                    echo "<p> <b class='recommended'>Recommended</b> <input type='radio' name='dauth' value=".$device_array_item['id'].' id="dauth'.$count.'" class="popup-label" ><label class="popup-label-text" for="dauth'.$count.'">'.$device_array_item['device_details'].'</label></p>';
                } else {
                    echo "<p><b class='recommended'> &nbsp; </b> <input type='radio' name='dauth' value=".$device_array_item['id'].' id="dauth'.$count.'" class="popup-label" ><label class="popup-label-text" for="dauth'.$count.'">'.$device_array_item['device_details'].'</label></p>';
                }
                $count++;  
           } 
