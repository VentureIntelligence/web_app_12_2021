...
<?php
/* Industryid, city and RegionI are added as part of the deal in RE */
					...
					foreach ($values as $i)
							{
								...
								//	echo "<br>-----------------------";
									if (trim($portfoliocompany) !="")
									{
										...
										$companyId=insert_company($portfoliocompany,$CompanyIndustryId,$website,'IN',1);
										if($companyId==0)
										{
											$companyId=insert_company($portfoliocompany,$CompanyIndustryId,$website,'IN',1);
										}
									//echo "<Br>COMPANYID===========".$companyId;
										...
										if ($companyId >0)
										{
											$pegetInvestmentsql = "select c.PECompanyId,ma.PECompanyId,dates from REcompanies as c,
											REinvestments as ma where ma.PECompanyId = c.PECompanyId and
											ma.dates = '$fullDateAfter' and c.PECompanyId = $companyId and Deleted=0";
											...
											if($investment_cnt==0)
											{
													...
													$insertcompanysql= "INSERT INTO REinvestments (PEId,PECompanyId,dates,amount,round,StageId,stakepercentage,comment,MoreInfor,Validation,InvestorType,Deleted,hideamount,hidestake,SPV,CreatedDate,ModifiedDate,city,RegionId,sector,IndustryId)
													VALUES ($PEId,$companyId,'$fullDateAfter',$DealAmount,'$Round',$RETypeId,$stakepercentage,'$comment','$moreinfor', '$validation','$investortype',$flagdeletion,$hideamount,$hidestake,$flagSPV,'$createddate','$modifieddate','$city',$RegionId,'$sector_focus',$indid)";
														...
														//echo "<br>Insert PE-" .$insertcompanysql;
														 foreach ($investorString as $inv)
															...
															foreach($advisor_companyString as $advisorcompany)
																{
																	$advisorcompany=str_replace('"','',$advisorcompany);
		...
		function insert_get_CIAs($cianame)
		{
			...
			if ($rsgetInvestorId = mysql_query($getInvestorIdSql))
			{
				...
				if ($investor_cnt==0)
				{
						...
						if($rsInsAcquirer = mysql_query($insAcquirerSql))
						{
							echo "<Br>Insert Advisor..check for Random no--" .$insAcquirerSql;
							$ciaInvestorId=0;
...
function returnMonth($mthname)
		...
		function returnDate($mth,$yr)
		{
		  ...
		  //  echo "<Br>".trim($yr)."-".trim($mth);
			...
			$fulldate="20".trim($yr)."-".trim($mth)."-01";
			//echo "<br>%%%%%%%%%%%%" .$fulldate;
