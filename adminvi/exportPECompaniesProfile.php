				     ...
				     while($row = mysql_fetch_row($result))
				     {
						...
						if(($invResult==0) && ($invResult1==0) && ($invResult2==0))
						{
							...
							$onBoardSql="select pec.PECompanyId,mgmt.PECompanyId,mgmt.ExecutiveId,
								...
								$companyId=$row[0];//CompanyId
								$schema_insert .=$row[2].$sep; //Companyname
								$industrysql="select Industry from industry where industryid=".$row[3];
								$industryquery=mysql_query($industrysql);
								if($row1=mysql_fetch_row($industryquery))
								{
								$industryvalue =$row1[0]; //Industry
								}
								$schema_insert .=$industryvalue.$sep; //Industry
								$schema_insert .=$row[4].$sep; //sector
								$schema_insert .=$row[11].$sep; //Stock code
								$schema_insert .=$row[25].$sep; //CIN no
								...
								$schema_insert .=$row[10].$sep; //Region
								$countrysql="select country from country where countryid='".$row[19]."'";
								
								$countryquery=mysql_query($countrysql);
								if($row2=mysql_fetch_row($countryquery))
								{
									
								$countryvalue =$row2[0];
								}

								$schema_insert .= $countryvalue.$sep; //Country
								...
								$schema_insert .=$row[23].$sep; //Moreinformation
