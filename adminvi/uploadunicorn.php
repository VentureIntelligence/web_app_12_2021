...
require("checkaccess.php");
  checkaccess( 'upload_unicornTrack' );
...
function overrideImport(value){
    if(value == "Yes"){
        $(".loader").show();
        $("#popup_main").hide();
        var formData = new FormData($('#unicornfile')[0]);
        $.ajax({
            url: 'unicornimport.php?override=true',
            type: "POST",
            data: formData,
            processData: false,
            contentType: false,
            success: function(data){
                ...
                $('#fileexists').hide();
                $('.ip-file').val('');
                $('#textsucess').html("Unicorn Table is Updated Successfully !!!");
            },
            error:function(){
                $(".loader").hide();
                $("#popup_main").hide();
...
function getUnicornImport()
{
    if (document.dealsupload.unicornfilepath.value == "")
    {
                             ...
                             <div id="maintextpro" style="background-color: #ffffff;width:100%; " class="main_content_container">
                                <div id="headingtextpro">
                                    <div class="acc-section">
                                        <form name="dealsupload" enctype="multipart/form-data" id="unicornfile" method="post" >
                                            <div class="accordian">
                                                <h3 class="acc-title"><span>Unicorn Table</span> <i class="zmdi zmdi-chevron-down"></i></h3>
                                                <div class="acc-content">
                                                    <p>Upload and .xlsx file through the window:</p>
                                                    <div class="upload-sec">
                                                        <input type="file" name="unicornfilepath" class="ip-file">
                                                    </div>
                                                    <div class="btn-sec text-right">
                                                        <input type="button" class="btn" value="Upload" onClick="getUnicornImport();">
                                                    </div>
                                                </div>
                                            </div>
                                        </form>
