...
<body>
<form name="deletelogs"  method="post" action="" >
	  ...
	  <div style="background-color:#FFF; width:565px; height:405px; margin-top:5px;">
	    <div id="maintextpro">
	     <div id="headingtextpro">
			<?php
			$deletelogkeyword=$_POST['DelLog'];
			//echo "<br>--" .$deletelogkeyword;
			...
			<?php
			foreach ($deletelogkeyword as $deletelogkey)
			{
				//echo "<br>^^^^" .$deletelogkey;
				$splitkeyword=explode("+",$deletelogkey);
				$emailid=trim($splitkeyword[0]);
				$login=trim($splitkeyword[1]);
				$emailid=str_replace(' ','',$emailid);

				$dellogquery= "delete from dealLog where EmailId='$emailid' and LoggedIn='$login'";
				//echo "<br>--" .$dellogquery;
					if ($delrs=mysql_query($dellogquery))
					{
			?>
						<tr bgcolor="#00CC66" style="font-family: Verdana; font-size: 8pt">
						<td> <?php echo $emailid; ?> <?php echo $login;?> -- Deleted </td> </tr>
			<?php
					}
					else
					{
			?>
						...
						<td> <?php echo $emailid; ?> <?php echo $login;?> -- Delete Failed </td> </tr>
					<?php
					}
