        ...
        if(!$_FILES['leaguefilepath']['error'])
        {
            ...
            if($uploadOk != 0){
                if (move_uploaded_file($inputFile, $target_file)) {
                    ...
                    mysql_query("TRUNCATE table leaguetable_main_data");
                    ...
                    for($i=3;$i<count($dataleague); $i++){
                        ...
                        $select_Query = "SELECT id FROM `leaguetable_main_data` WHERE advisor_name='$advisorname' AND deal='$deal' AND amount='$amt' AND industry='$industry' AND sector='$sector' AND date='$date_deal' AND deal_type='$dealtype' AND points='$points' AND advisor_type='$advisor_type'";
                        $exec_sel = mysql_query($select_Query);
                        $selcnt = mysql_num_rows($exec_sel);
                        if($selcnt == 0){
                            $rowcount++;
                           // if((count($dataleague[$i]) == '10') || (count($dataleague[$i]) == '11')){
                            $insert_Query = "INSERT INTO `leaguetable_main_data` (`id`, `advisor_name`, `deal`, `amount`, `industry`, `sector`, `date`, `deal_type`, `points`, `advisor_type`,`notable`,`created_date`) 
                    ...
                    $select_Query1 = "SELECT id FROM `leaguetable_main_data` ";
                   ...
                   // rsort($Lyears);
                    ...
                    if($inputFilename !="" || $rowcount !="" || $tablecount !="" ||  $latestyear !="" ||  $username !="" ){
                           ...
                           $insert_Query = "INSERT INTO `leaguetable_main_log` (`username`, `logfile`, `excel_total_rows`, `table_total_rows`, `latestyear`, `created_date`) 
