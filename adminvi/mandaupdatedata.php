		...
		if($filename!="")
		{
    	 ...
    	 $UpdatecompanySql="update pecompanies set companyname='$companyNametoUpdate',industry=$industrytoUpdate,sector_business='$sectortoUpdate',website='$websiteToUpdate',modefied_by='$user' where PECompanyId=$companyIdtoUpdate";
			...
			if($updaters=mysql_query($UpdatecompanySql))
			{
				...
				Validation='$validationtoUpdate',InvestorType='$invTypeId',type='$intype',hideamount=$hideamouttoUpdate, hidemoreinfor=$hidemoreinfortoUpdate,VcFlag=$vcflagtoUpdate,
				...
				Valuation='$valuation',FinLink='$finlink', uploadfilename='$filename',
                                ...
                                EBITDA_Multiple=$ebitda_mutliple,PAT_Multiple=$pat_mutliple, price_to_book=$price_to_book, book_value_per_share=$book_value_per_share, price_per_share=$price_per_share, ExitStatus=$exitstatusvalue,Revenue=$revenue,EBITDA=$ebitda,PAT=$pat,stakeSold=$stakesold
				where MandAId=$MandAIdtoUpdate";
				echo "<br>Query--**-- " .$UpdateInvestmentSql;
				 $UpdateSectorSql="update manda AS pe,pe_sectors as pes,pe_subsectors as pess SET pess.subsector_name='$subsector',pes.sector_name='$mainsector',pess.Additional_subsector='$addsubsector' where pe.MandAId =" .$MandAIdtoUpdate." and pe.PECompanyId=pess.PECompanyId and pess.sector_id=pes.sector_id";
						...
						if ($investoridarray>0)
							{
								...
								for ($i=0;$i<=$investoridarray-1;$i++)
								{
  								 ...
  								 $investorString=explode(",",$investorswithMultipletoUpdate);
																...
																$returntoIRR=$investorString[2];
                                                                //echo "<br>--- " .$returntoIRR;exit();
								...
								//  if($investortoUpdate!="")
                                //                                 {
                                //                                   $updateInvReturnSql= "Update manda_investors set MultipleReturn=$returntoUpdate,IRR=$returntoIRR,InvMoreInfo='$invmoreinfostring' where MandAId=$MandAIdtoUpdate and InvestorId=$investorIdtoUpdate";
