<?php
    ...
    if ($rsccompany = mysql_query($getCompanySql))
    {
        While($myrow=mysql_fetch_array($rsccompany, MYSQL_BOTH))
        {
            ...
            $jsonarray[]=array('id'=>$companyId,'label'=>$companyName,'value'=>$companyNamelower,'industry'=>$industry,'city'=>$city, 'state'=>$state, 'stateid'=>$stateid, 'RegionId' =>$RegionId, 'sector_business' =>$sector_business);
        }
    }
