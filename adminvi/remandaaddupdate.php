				...
				if (trim($portfoliocompany) !="")
				{
					...
					if ($companyId >0)
					{
						...
						if($investment_cnt==0)
						{
								...
								$insertcompanysql="";
								$insertcompanysql= "INSERT INTO REmanda (MandAId,PECompanyId,AcquirerId,DealTypeId,DealDate,DealAmount,Comment,MoreInfor,Validation,Deleted,hidemoreinfor,hideamount,city,RegionId,InvestmentDeals,Link,EstimatedIRR,MoreInfoReturns,StageId,SPV,ProjectName,ExitStatus)
								VALUES ($MandAId,$companyId,$AcquirerId,$dealtypeId,'$fullDateAfter',$MandASize,'$comment','$moreinfor', '$validation',$flagdeletion,$hidemoreinfor,$hideamount,'$city',$regionId,'$invdealsummary','$link','$estimatedirr','$moreinforeturns',$StageId,$SPVtoAdd,'$projectname','$exitstatusvalue')";
...
function returnAcquirerId($acquirername)
{
	...
	$getAcquirerSql="select AcquirerId from REacquirers where Acquirer = '$acquirername'";
