		...
		//	echo "<Br>Company - " .$companyName;
			$UpdateCompSql= "Update dealcompanies set DCompanyName='$companyName',ExpiryDate='$expDate',TrialLogin=$trial, PEInv=$PEInv,VCInv=$VCInv,REInv=$REInv,PEIpo=$PEIpo,VCIpo=$VCIpo,
				PEMa=$PEMa,VCMa=$VCMa,PEDir=$PEDir,VCDir=$CODir,SPDir=$SPDir,MAMA=$MaMa,Inc=$Inc,AngelInv=$angelInv,SVInv=$sv,IfTech=$itech,CTech=$ctech,
                                 Student=$student,REInv=$relogin,IPAdd=$ipFlag,poc='$contacts',permission='$permission'
			...
			//echo "<br>--" .$UpdateCompSql;
                        ...
                        for ($i=0;$i<count($_POST['ipAddress']);$i++){
                            ...
                            //Check if IP already exists for the company
                            /*$sqlCheckCompIp = "SELECT `ipAddress` FROM `ipAddressKey` WHERE `DCompId`='".$companyId."'";
                            $resCheckCompIP = mysql_query($sqlCheckCompIp) or die(mysql_error());
                            $cntCheckCompIp = mysql_num_rows($resCheckCompIP);
                            
                            //Get the max id of slno field from `ipAddressKey`
                            $sqlGetMaxSno = "SELECT max(slno) FROM `ipAddressKey`";
                            $resGetMaxSno = mysql_query($sqlGetMaxSno) or die(mysql_error());
                            $valGetMaxSno = mysql_fetch_row($resGetMaxSno);
                                                       
                            //print_r($_POST);
                            $ipAddress = $_POST['ipAddress'];
                            $startRng = $_POST['startRange'];
                            $endRng = $_POST['endRange'];
                            
                            //Insert or Update IP Address Details
                            if (trim($ipAddress)!=''){
                                if ($cntCheckCompIp==0){
                                    //Insert IP
                                    $sNo = $valGetMaxSno[0]+1;
                                    ...
                                    //Update IP
                                    $sqlUpdIpPng = "UPDATE `ipAddressKey` SET `DCompId`='".$companyId."',`ipAddress`='".$ipAddress."',`StartRange`='".$startRng."',`EndRange`='".$endRng."' WHERE `DCompId`='".$companyId."'";
                                    mysql_query($sqlUpdIpPng) or die(mysql_error());
                                }
                            }*/                          
                        //END
