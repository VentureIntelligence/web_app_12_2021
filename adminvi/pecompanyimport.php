				...
				foreach ($values as $i)
				{
					...
					if($i != "")
					{
						...
						if (trim($portfoliocompany) !="")
						{
							...
							$companyId=insert_company($portfoliocompany,$indid,$sector,$website,$stock,$yearfounded,$address1,$address2,
								$adcity,$zip,$otherlocation,$country,$telephone,$fax,$email,$addinfor);
...
/* function to insert the companies and return the company id if exists */
	function insert_company($companyname,$industryId,$sector,$web,$stockcode,$yearfounded,$add1,$add2,$adcity,$zip,$otherloc,$country,$tel,$fax,$email,$addinfor)
	{
    ...
    //	echo "<br>select--" .$getPECompanySql;
		if ($rsgetPECompanyId = mysql_query($getPECompanySql))
		{
			...
			if ($pecomp_cnt==0)
			{
					...
					pecompanies(companyname,industry,sector_business,website,stockcode,yearfounded,Address1,Address2,AdCity,Zip,OtherLocation,Country,Telephone,Fax,Email,AdditionalInfor)
					values('$companyname',$industryId,'$sector','$web','$stockcode','$yearfounded','$add1','$add2','$adcity','$zip','$otherloc','$country','$tel','$fax','$email','$addinfor')";
			...
			elseif($pecomp_cnt==1)
			{
				...
				//industry=$industryId,  sector_business='$sector',website='$web',
					...
					Fax='$fax',Email='$email',AdditionalInfor='$addinfor' where
