				...
				$pwd=trim($pwd);
                                ...
                                if((trim($login)== "") && (trim($pwd)=="")){
                                    ...
                                    $checkForIPAddress="SELECT ip.DCompId, dm.EmailId, dm.Passwrd, dm.Name,ip.StartRange,ip.EndRange
                                            FROM ipAddressKey AS ip, RElogin_members AS dm,dealcompanies as dc
					...
					if($random_cnt==1)
					{
						/*$checkUserSql= "SELECT dm.EmailId, dm.Passwrd,dm.Name, dm.DCompId,dc.ExpiryDate FROM RElogin_members AS dm,
										...
										AND dc.Deleted =0 and IPAdd=0";*/
                                                ...
                                                $checkUserSql= "SELECT dm.EmailId, dm.Passwrd,dm.Name, dm.DCompId,dc.ExpiryDate,dc.IPAdd,dm.deviceCount,dm.exportLimit,dc.Student FROM RElogin_members AS dm,
										dealcompanies AS dc WHERE dm.DCompId = dc.DCompId AND
										dm.EmailId='$login' and dm.Passwrd='$pwd'
										AND dc.Deleted =0";
...
//                                                                                if ($cntChkCookie==0)
                                                                                ...
                                                                                if ($myrow['Student']==0){
                                                                                    ...
                                                                                    $displayMessage = 'Login Error - Device not authorized. Please contact your administrator for Authorization Code. <a href="/re/auth.php">Authorize</a>';
									...
									if( date('Y-m-d')<=$myrow["ExpiryDate"])
									{
										...
										//{
											...
											//header( 'Location: /re/reindex.php' ) ;
                                                                                        header( 'Location: /re/reindex.php' ) ;
										//}
									}
...
function sendAuthEmail($companyId,$userEmail,$allowedDevices){
    ...
    $devicesUsed = getDevicesUsedCount($userEmail,'RE');
    //Send Email
...
<form name="relogin" onSubmit="return checkFields();" method="post" action="reloginip.php" >
...
<ul>
    ...
    <li><input type="submit" name="btnSubmit" value ="Login" class="fp"/> <a href="<?php echo BASE_URL; ?>re/forgetpwd.php?value=R" class="forgot-link">Forgot your password?</a> </li>
