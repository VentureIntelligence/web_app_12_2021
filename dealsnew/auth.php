...
if ($_POST['btnSubmit']=='Authorize'){
    ...
    if ($displayMessage==''){
        ...
        $sqlAuthCheck = "SELECT `expDate` FROM `user_auth_code` WHERE `emailId`='".$userEmail."' AND `dbType`='PE' AND `authCode`='$authCode' AND `expDate` >= '".$today."'";
        $resAuthCheck = mysql_query($sqlAuthCheck);
        $cntAuthCheck = mysql_num_rows($resAuthCheck);
        
        $REsqlAuthCheck = "SELECT `expDate` FROM `user_auth_code` WHERE `emailId`='".$userEmail."' AND `dbType`='RE' AND `authCode`='$authCode' AND `expDate` >= '".$today."'";
        $REresAuthCheck = mysql_query($REsqlAuthCheck);
        $REcntAuthCheck = mysql_num_rows($REresAuthCheck);
        
        //Check if the auth code is correct
        $MAsqlAuthCheck = "SELECT `expDate` FROM `user_auth_code` WHERE `emailId`='".$userEmail."' AND `dbType`='MA' AND `authCode`='$authCode' AND `expDate` >= '".$today."'";
        $MAresAuthCheck = mysql_query($MAsqlAuthCheck);
        $MAcntAuthCheck = mysql_num_rows($MAresAuthCheck);
        
         $currPECookie = $_COOKIE['peLoginAuth'];
         $currPECookieArray=  json_decode($currPECookie,true);
         
         $currRECookie = $_COOKIE['reLoginAuth'];
         $currRECookieArray=  json_decode($currRECookie,true);
         
         $currMACookie = $_COOKIE['maLoginAuth'];
         $currMACookieArray=  json_decode($currMACookie,true);
        ...
        if ($cntAuthCheck==1 ){
            ...
            //echo $devicesUsed . '<=' .$allowedNoDevices;
            if ($devicesUsed <= $allowedNoDevices){
                ...
                //Message to login
                /*$successMessage = 'You have successfully authorized this device. <a href="pelogin.php">Login Here</a>';
                $userEmail = '';
                $authCode  = '';*/
             //   header("location:pelogin.php?auth=1");
            ...
            mysql_query($sqlmauserstatus) or die(mysql_error());
                ...
                header( 'Location: ' . BASE_URL . 'pelogin.php?auth=1');
                exit;
...
<div class="header"><img src="images/logo.png" width="167" height="45" alt="Venture Intelligence" title="Venture Intelligence" border="0" /></div>


<div class="login-cnt">

<form name="pelogin" onSubmit="return checkFields();" method="post" action="" >
 <p style="color:#F10000;font-weight: bold;background: #fff;width:93%;padding: 1% 4%;border-radius: 2px;font-size: 13px;line-height: 16px;">Please await email from Venture Intelligence (in your registered email id) containing your authorization code.</br><small>Please check your spam/junk folder also.</small></p>   
    <ul>
...
<li><label style="width:100%;">Authorization Code</label> <input type=password name="authCode" value="<?php echo $authCode; ?>" size="23" style="padding: 5px;"/></li>
...
<li><font color="green"><?php echo $successMessage; ?></font></li>
 ...
 <!--   <div style="width: 1000px;margin: 10px auto;padding:50px 0; ">
        <table cellpadding="5" cellspacing="0">
            <tr><td>Email</td><td><input type=text name="emailid" value="" size="23" style="padding: 5px;"/></td></tr>
            <tr><td>Password</td><td><input type=password name="emailpassword" value="" size="23" style="padding: 5px;"/></td></tr>
            <tr><td colspan="2"><input type="checkbox" name="chkTerm" CHECKED />
        	By accessing this database, you agree to the <a href="../terms.htm">terms & conditions</a>
			of use upon which access is provided.</td></tr>
            <tr><td></td><td><input type="submit" name="btnSubmit" value ="Login"/></td></tr>
            <tr><td align="center" style="color: #ff0033;" colspan="2"><?php echo $displayMessage; ?></td></tr>
            <tr><td colspan="2"><a href="forgetpwd.php?value=P">Forgot Password ?</a></td></tr>
        </table>
    </div> -->
</form>
