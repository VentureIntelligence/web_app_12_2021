...
(SELECT count(inv.Investor) FROM peinvestments_investors as peinv_inv,peinvestors as inv WHERE   peinv_inv.PEId=pe.PEId and inv.InvestorId=peinv_inv.InvestorId ) AS Investorcount
                                ...
                                JOIN stage AS s ON s.StageId=pe.StageId   WHERE  peinv_inv.InvestorId IN(".$keyword.") and  dates between '".$startDate."' and '".$endYear."' and  pe.Deleted=0  and pec.industry !=15  
                            AND pe.PEId NOT
                            IN (
                            SELECT PEId
                            ...
                            )  AND pec.industry IN (49, 14, 9, 25, 24, 7, 4, 16, 17, 23, 3, 21, 1, 2, 10, 54, 18, 11, 66, 106, 8, 12, 22)   GROUP BY pe.PEId order by  dates desc,companyname asc ";
...
$DataList = array();
$col = 0;  
    //echo json_encode($rows['PEId']);exit();
    if(isset($rows['PEId'])){
        $PEId = $rows['PEId'];
    }else{
        $PEId = $rows[13];
