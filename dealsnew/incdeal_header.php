...
<label  id="vctour4"><input class="investment-nav" name="investments" type="radio" data-name="INVESTMENT - SOCIAL" value=3  <?php if($vcflagValue==3) { ?> checked="checked" <?php } ?>/>Social VC</label></div>
</div>
 -->
...
if($_POST['month1']!='' && $_POST['year1']!='' && $_POST['month2']!='' && $_POST['year2']!=''){
      if($_POST['year1']!='')
        ...
        if($_POST['month2']!='')
       {
        ...
        $sdatevalueDisplay1 = returnMonthname($month1) ." ".date('y', strtotime($getdt1));
        $edatevalueDisplay2 = returnMonthname($month2) ."  ".date('y', strtotime($getdt2));
} else if($_POST['month1'] =='' && $_POST['year1'] =='' && $_POST['month2'] =='' && $_POST['year2'] ==''){
    $month1 = date('n');
    $month2 = date('n');
    $year1 = date('Y', strtotime(date('Y')." -1  Year"));
    $year2 = date('Y');
    $sdatevalueDisplay1 = returnMonthname($month1) ." ".$year1;
    $edatevalueDisplay2 = returnMonthname($month2) ."  ".$year2;
}
?>
