				...
				*/
				...
				$addVCFlagqry = " and pec.industry !=15 ";
   				$searchTitle = "List of Incubator Deals";
                                ...
                                if (($keyword == "") && ($companysearch=="") && ($industry =="--")  && 	($statusid == "--") && ($followonFund == "--") &&($regionid=="") && ($incfirmtypeid==0) && ($city==''))
				{
                                        ...
                                        pe.YearFounded,pec.website, pec.AdCity, pec.region,pec.RegionId,MoreInfor,pe.StatusId,s.Status,FollowonFund, pe.date_month_year
                                        ...
                                        AND pec.PEcompanyID = pe.IncubateeId  and pe.Deleted=0" .$addVCFlagqry.$addDefunctqry. " ".$addDelind." order by companyname";
							...
							AND pe.Deleted =0 " .$addVCFlagqry.$addDefunctqry. " ".$addDelind." AND  ( $tagsval ) GROUP BY pe.IncDealId  order by companyname"; 
				...
				elseif ($companysearch != "")
                                {
                                        ...
                                        WHERE $datefilter pec.industry = i.industryid AND pec.PEcompanyID = pe.IncubateeId
                                        and inc.IncubatorId=pe.IncubatorId  and s.StatusId=pe.StatusId
                                        AND pe.Deleted =0 " .$addVCFlagqry.$addDefunctqry. " ".$addDelind." AND  pec.PECompanyId IN ($companysearch)
				...
				elseif($keyword!="")
                                {
                                        ...
                                        and pec.PECompanyId=pe.IncubateeId " .$addVCFlagqry.$addDefunctqry." ".$addDelind." AND inc.IncubatorId IN($keyword) order by companyname";

                                        //echo "<br> Investor search- ".$companysql;
                                }

                                elseif (($industry > 0) || ($statusid >0) ||  ($followonFund >=0) || ($regionid> 0) || ($incfirmtypeid >0) || ($city!='') )
                                {
                                        $companysql = "select pe.IncDealId,pe.IncubateeId,pec.companyname,pec.industry,i.industry,pec.sector_business,
                                        Individual,inc.Incubator,pe.YearFounded,pec.website, pec.AdCity, pec.region,pec.RegionId,MoreInfor,pe.StatusId,s.Status,FollowonFund, pe.date_month_year
                                        from incubatordeals as pe, industry as i,pecompanies as pec,incubators as inc,incstatus as s
                                        where $datefilter ";
