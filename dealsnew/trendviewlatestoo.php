        ...
        elseif($vcflagValue==1)
        {
           ...
           // print_r($_POST);
				...
				$industry=0;
                                ...
                                elseif ($type==2) 
                                {
                                   $companysql = "SELECT i.industry,YEAR(pe.dates) , COUNT(distinct pe.PEId) , SUM(distinct pe.amount) from peinvestments as pe, industry as i,pecompanies as pec where i.industryid=pec.industry and pec.PEcompanyID = pe.PECompanyID and  pe.Deleted = 0 AND pe.AggHide = 0 AND pe.SPV=0 and  dates between '".$startyear."' and '".$endyear."' AND pe.PEId NOT IN (SELECT PEId FROM peinvestments_dbtypes AS db WHERE hide_pevc_flag =1) GROUP BY i.industry,YEAR(pe.dates)";
                                 //echo  $companysql;
                                ...
                                elseif($type==3)
                                {
                                   $companysql = "SELECT s.Stage,YEAR(pe.dates) , COUNT(distinct pe.PEId) , SUM(distinct pe.amount) from peinvestments as pe, industry as i,pecompanies as pec,stage as s where i.industryid=pec.industry and pec.PEcompanyID = pe.PECompanyID and pe.StageId = s.StageId and  pe.Deleted = 0 AND pe.AggHide = 0 AND pe.SPV=0 and  dates between '".$startyear."' and '".$endyear."' AND pe.PEId NOT IN ( SELECT PEId FROM peinvestments_dbtypes AS db WHERE hide_pevc_flag =1) GROUP BY s.Stage,YEAR(pe.dates) ";   
                                   //echo  $companysql;
                                   //$resultcompany= mysql_query($companysql);
                                }
                                ...
                                else if($type ==4)
                                {
                                    ...
                                    for($r=0;$r<count($range);$r++)
                                    {
                                        if($r == count($range)-1)
                                        {
                                            $companysql = "SELECT YEAR( pe.dates ) , COUNT(distinct pe.PEId ) , SUM(distinct pe.amount ) FROM peinvestments AS pe, industry AS i, pecompanies AS pec WHERE i.industryid = pec.industry AND pec.PEcompanyID = pe.PECompanyID and  (pe.amount > 200) AND pe.Deleted =0 " .$addVCFlagqry. " AND pe.AggHide =0 AND pe.SPV =0 and dates between '".$startyear."' and '".$endyear."' AND pe.PEId NOT IN (SELECT PEId FROM peinvestments_dbtypes AS db WHERE hide_pevc_flag =1) GROUP BY YEAR( pe.dates )";
                                            //echo  $companysql;
                                             //$resultcompany= mysql_query($companysql);
                                        }
                                        else
                                        {
                                            ...
                                            $companysql = "SELECT YEAR( pe.dates ) , COUNT(distinct pe.PEId ) , SUM(distinct pe.amount ) FROM peinvestments AS pe, industry AS i, pecompanies AS pec WHERE i.industryid = pec.industry AND pec.PEcompanyID = pe.PECompanyID and  (pe.amount > ".$elimit[0]." and pe.amount<= ".$elimit[1].") AND pe.Deleted =0" .$addVCFlagqry. " AND pe.AggHide =0 AND pe.SPV =0 and dates between '".$startyear."' and '".$endyear."' AND pe.PEId NOT IN (SELECT PEId FROM peinvestments_dbtypes AS db WHERE hide_pevc_flag =1) GROUP BY YEAR( pe.dates )";
                                           // echo  $companysql;
                                            //$resultcompany= mysql_query($companysql);
                                        }
                                ...
                                elseif($type==5)
                                {
                                    $companysql = "SELECT inv.InvestorTypeName,YEAR(pe.dates) , COUNT(distinct pe.PEId) , SUM(distinct pe.amount) from peinvestments as pe, industry as i,pecompanies as pec,stage as s,investortype as inv where i.industryid=pec.industry and pec.PEcompanyID = pe.PECompanyID and pe.StageId = s.StageId and  pe.Deleted = 0 " .$addVCFlagqry. " AND pe.AggHide = 0 AND pe.SPV=0 and pe.InvestorType=inv.InvestorType and  dates between '".$startyear."' and '".$endyear."' AND pe.PEId NOT IN (SELECT PEId FROM peinvestments_dbtypes AS db WHERE hide_pevc_flag =1) GROUP BY inv.InvestorTypeName,YEAR(pe.dates) ";
                                   // echo  $companysql;
                                  //$resultcompany= mysql_query($companysql);
                                }
                                elseif($type==6)
                                {
                                    $companysql  = "SELECT r.Region,YEAR(pe.dates) , COUNT(distinct pe.PEId) , SUM(distinct pe.amount) from peinvestments as pe, industry as i,pecompanies as pec,stage as s,investortype as inv, region as r where i.industryid=pec.industry and pec.PEcompanyID = pe.PECompanyID and  r.RegionId=pec.RegionId and pe.StageId = s.StageId and  pe.Deleted = 0 " .$addVCFlagqry. " AND pe.AggHide = 0 AND pe.SPV=0 and pe.InvestorType=inv.InvestorType and dates between '".$startyear."' and '".$endyear."' AND pe.PEId NOT IN ( SELECT PEId FROM peinvestments_dbtypes AS db WHERE hide_pevc_flag =1) GROUP BY r.Region,YEAR(pe.dates)";
                                    //echo  $companysql;
				...
				//echo "<br> individual where clauses have to be merged ";
					if ($industry > 0)
                                ...
                                //	echo "<br>----comp sql after region-- " .$companysql;
                                        ...
                                        {
                                                $companysql.="( city LIKE '$searchallfield%' or pec.companyname LIKE '$searchallfield%'	OR sector_business LIKE '$searchallfield%' or MoreInfor LIKE '$searchallfield%' ) AND";
                                        }
                                        if(($wheredates !== "") )
                                        {
                                                $companysql = $companysql ." pe.Deleted=0 AND pe.AggHide =0 AND pe.SPV =0 and pec.industry = i.industryid and pec.PEcompanyID = pe.PECompanyID and pe.StageId=s.StageId " . $addVCFlagqry . "  AND ".$wheredates ." and ";
												 
						$companysql2 = $companysql2 ." pe.Deleted=0 AND pe.AggHide =0 AND pe.SPV =0 and pec.industry = i.industryid and pec.PEcompanyID = pe.PECompanyID and pe.StageId=s.StageId " . $addVCFlagqry . "  AND ".$wheredates ." and ";
