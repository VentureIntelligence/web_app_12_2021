...
function updateDownload($res){
   ...
   if ($rowSelCount > 0){
    ...
    elseif($type==3)
    {
         $Tabletitle="PE - By Stage";
         $filename="PE_By_Stage";
    ...
    elseif($type==5)
    {
         $Tabletitle="PE - By Investor";
         $filename="PE_By_Investor";
    ...
    elseif($type==6)
    {
       ...
       $sqlregion = "SELECT r.Region,YEAR(pe.dates) , COUNT(pe.PEId) , SUM(pe.amount) 
					from peinvestments as pe, industry as i,pecompanies as pec,stage as s,investortype as inv, region as r where i.industryid=pec.industry and
						pec.PEcompanyID = pe.PECompanyID and pec.RegionId=r.RegionId and pe.StageId = s.StageId and  pe.Deleted = 0 AND pe.AggHide = 0 AND pe.SPV=0
						and pe.InvestorType=inv.InvestorType and dates between '".$startyear."' and '".$endyear."'
                                                AND pe.PEId NOT
                                                IN (
                                                SELECT PEId
 ...
 */
    ...
    while($rowstage = mysql_fetch_array($result))	
    {  
