...
if (session_is_registered("SessLoggedAdminPwd") && session_is_registered("SessLoggedIpAdd"))
	 	{
			...
			//	$targetcityId=insert_city($targetcity);
				...
				if (trim($portfoliocompany) !="")
				{
					...
					$companyId=insert_company($portfoliocompany,$indid,$sector,$website,$targetCountryId,$targetcity,$regionId);
					if($companyId==0)
					{
						$companyId=insert_company($portfoliocompany,$indid,$sector,$website,$targetCountryId,$targetcity,$regionId);
					}
					...
					if ($companyId >0)
					{
						...
						if($investment_cnt==0)
						{
								...
								$insertcompanysql= "INSERT INTO mama (MAMAId,PECompanyId,Amount,Stake,DealDate,MADealTypeId,AcquirerId,Comment,MoreInfor,Validation,Asset,Deleted,CreatedDate,ModifiedDate,hideamount,Link,uploadfilename,source,Valuation,FinLink,Company_Valuation,Revenue_Multiple,EBITDA_Multiple,PAT_Multiple,target_listing_status,acquirer_listing_status)
								VALUES ($MAMAId,$companyId,$amount,$stake,'$fullDateAfter',$dealTypeId,$AcquirerId,'$comment','$moreinfor', '$validation',$assetFlag,$flagdeletion,'$createddate','$modifieddate',$hideamountFlag,'$link','$filename','$sourcename','$valuation','$finlink',$company_valuation,$revenue_multiple,$ebitda_multiple,$pat_mutliple,'$target_listingstatusvalue','$acquirer_listingstatusvalue')";
...
/* function to insert the companies and return the company id if exists */
	function insert_company($companyname,$industryId,$sector,$web,$countryid,$cityid,$region)
	{
		...
		if ($rsgetPECompanyId = mysql_query($getPECompanySql))
		{
			...
			{
					//insert pecompanies
					$insPECompanySql="insert into pecompanies(companyname,industry,sector_business,website,countryid,city,RegionId)
					values('$companyname',$industryId,'$sector','$web','$countryid','$cityid',$region)";
			...
			elseif($pecomp_cnt>=1)
			{
				...
				{
					$companyId = $myrow[0];
					$updateCityCountrySql="Update pecompanies set city='$cityid',countryid='$countryid',RegionId=$region where PECompanyId=$companyId";
