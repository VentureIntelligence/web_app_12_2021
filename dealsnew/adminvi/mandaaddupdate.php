					...
					if (trim($portfoliocompany) !="")
					{
						...
						if ($companyId >0)
						{
							...
							if($investment_cnt==0)
							{
									...
									$insertcompanysql="";
									$insertcompanysql= "INSERT INTO manda (MandAId,PECompanyId,AcquirerId,DealTypeId,DealDate,DealAmount,Comment,MoreInfor,Validation,Deleted,InvestorType,type,hidemoreinfor,hideamount,VCFlag,InvestmentDeals,Link,EstimatedIRR,MoreInfoReturns,uploadfilename,source,Valuation,FinLink,Company_Valuation,Revenue_Multiple,EBITDA_Multiple,PAT_Multiple,ExitStatus)
									VALUES ($MandAId,$companyId,$AcquirerId,$dealtypeId,'$fullDateAfter',$MandASize,'$comment','$moreinfor', '$validation',$flagdeletion,'$investortype','$intype',$hidemoreinfor,$hideamount,$Vcflag,'$invdealsummary','$link','$estimatedirr','$moreinforeturns','$filename','$sourcename','$valuation','$finlink',$company_valuation,$revenue_multiple,$ebitda_mutliple,$pat_mutliple,$exitstatusvalue)";
