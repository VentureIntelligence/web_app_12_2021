...
<?php
	...
	$Db = new dbInvestments();
		...
		// echo "<Br>Investor String---------- ".$advisorInvestortoUpdate;
    	 ...
    	 $UpdatecompanySql="update REcompanies set companyname='$companyNametoUpdate',industry=$CompanyIndustryId,website='$urltoUpdate'
    	 where PECompanyId=$companyIdtoUpdate";
			...
			if($updaters=mysql_query($UpdatecompanySql))
			{
				...
				if($existingfile!="")
				{
					...
					listing_status='$listing_statusvalue'
				...
				elseif($existingfile=="")
				{
					...
					Valuation='$valuation',FinLink='$finlink',
                                        AggHide=$hideAggregatetoUpdate,ProjectName='$projectname',listing_status='$listing_statusvalue'
					where PEId=$PEIdtoUpdate";
				}
