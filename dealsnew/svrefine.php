    ...
    <div class="block">
        <ul > 
            ...
            <li class="tagpadding">
                ...
                <input type="button" name="fliter_stage" value="Filter" class="fliter_stage" style="float: right;">
            </li>
        </ul>
    </div>
</div>
...
<h2 class="acc_trigger"><a href="#">Refine Search</a></h2>
    ...
    <input type="hidden" name='filtersubsector' id="filtersubsector" value="<?php echo $filtersubsector; ?>" />
		<div class="block" id="firstrefinesv">
			<ul >
...
<li class="old"><h4>Round</h4>
    <div class="selectgroup">
        <select name="round[]" multiple="multiple" id="round" style=" background: <?php echo $background; ?>;" <?php if($disable_flag == "1"){ echo "disabled"; } ?>>
                <option id="100" value="bridge" <?php if(in_array("bridge",$round)) echo 'selected'; ?>>Bridge</option>
...
<li>
    <input type="button" name="fliter_stage" class="fliter_stage" value="Filter"  <?php if($disable_flag == "1"){ echo "disabled"; } ?>>
</li>
...
<li class="even"><h4>Deal Type</h4>
    ...
    <SELECT NAME="dealtype_debtequity" id="dealtype_debtequity" style=" background: <?php echo $background; ?>;" <?php if($disable_flag == "1"){ echo "disabled"; } ?>>
...
<li class="odd"><h4>Syndication</h4>
    ...
    <SELECT NAME="Syndication" id="Syndication" style=" background: <?php echo $background; ?> ;" <?php if($disable_flag == "1"){ echo "disabled"; } ?>>
...
<li  class="ui-widget" style="position: relative"><h4>State</h4>
                <div class="selectgroup">
                    ...
                    $statesql= "SELECT s.state_id, s.state_name FROM pecompanies as pec, state as s,peinvestments_dbtypes,peinvestments WHERE s.state_id=pec.stateid and pec.state !='' and peinvestments.PECompanyId=pec.PECompanyId and peinvestments_dbtypes.PEId=peinvestments.PEId  ".$dbtype." Group by s.state_id ORDER BY s.state_name ASC ";
                    echo "Stateval---".$statesql;
            ...
            <li  class="odd" style="position: relative"><h4>City</h4>
                <div class="selectgroup citysearch">
                
                    <select name="city[]" class="citylist" multiple="multiple"  id="citysearch" style=" background: <?php echo $background; ?>;" <?php if($disable_flag == "1"){ echo "disabled"; } ?>>
                    <?php
                        //  echo "vcflag----".$VCFlagValue;
                        if($VCFlagValue == 4){
                            $dbtype="and peinvestments_dbtypes.DBTypeId='CT'";
                        }elseif($VCFlagValue == 5){
                            $dbtype="and peinvestments_dbtypes.DBTypeId='IF'"; 
                        ...
                        if($statevalueid !=""){
                            $citysql = " SELECT DISTINCT city.city_id,city.city_name from city,pecompanies,peinvestments,peinvestments_dbtypes where pecompanies.PEcompanyID = peinvestments.PECompanyID and pecompanies.city=city.city_name and city.city_name!='' and city.city_name!='Not Identified - City' and city.city_name!='undefined' and city.city_StateID_FK IN (".$statevalueid.") and peinvestments_dbtypes.PEId=peinvestments.PEId ".$dbtype."  ORDER BY city.city_name ASC  ";
                        ...
                        if($ind_cnt > 0)
                        {
                            While($myrow=mysql_fetch_array($cityrs, MYSQL_BOTH))
                            {
                                ...
                                if(count($city)>0) {
                                    ...
                                    echo "<option id='city_".$id. "' value=".$id.">".ucwords(strtolower($name)) ."</option>\n";
                            }
                        }
...
<input type="text " name="invrangestart" id="invrangestart" maxlength="5" autocomplete="off" value="<?php echo $startRangeValue;?>" style="    border-width: 1px; float:left;width:41%; background: <?php echo $background; ?>;" <?php if($disable_flag == "1"){ echo "disabled"; } ?> onkeypress="return isNumberKey1(event)" size="11">
    
<span class="range-text"> to</span>
<input type="text " name="invrangeend" id="invrangeend" maxlength="5" autocomplete="off" value="<?php echo $endRangeValue;?>"  style="    border-width: 1px;float:left;width:41%; background: <?php echo $background; ?>;" <?php if($disable_flag == "1"){ echo "disabled"; } ?> onkeypress="return isNumberKey1(event)" size="11">
...
function disableFileds(){
    ...
    $(' #dealtype_debtequity, #Syndication,#invType,#invhead,#invrangestart,#invrangeend,#sltstate').attr('style', 'background-color: #dddddd !important');
...
function enableFileds(){ 
    ...
    $('#dealtype_debtequity, #Syndication,#invType,#invhead,#invrangestart,#invrangeend,#sltstate').attr('style', 'background-color: #ffffff !important');
                ...
                success: function(data) {
                    ...
                    if(datacount>0) {
                        ...
                        $.each(innerdata, function (key, item) {
                            ...
                            multiselect+="<OPTION id='city_"+item['id']+ "' value="+item['id']+" >"+item['value']+"</OPTION>";
                        });
                        
                        $("#citysearch").append(multiselect);
                        $("#citysearch").multiselect('refresh');
                        $('.citysearch button.ui-multiselect').attr('style', 'width: 223px; background-color: #fff !important').prop("disabled",false);
                    }
                }
            });
        // }
        // else{
        //     $('.citysearch button.ui-multiselect').attr('style', 'width: 223px; background-color: #dddddd !important').prop("disabled",true);
        //     $("#citysearch").empty();
        //     $("#citysearch").multiselect('refresh');
        // }
    });
