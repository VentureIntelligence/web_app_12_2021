...
<select >  
<option value="investmentspe" <?php echo ($vcflagValue==0) ? 'SELECTED' : ""; ?>>PE</option>
<option value="investmentsvc" <?php echo ($vcflagValue==2 || $vcflagValue==1 || $vcflagValue==6) ? 'SELECTED' : ""; ?> >VC</option>
</select>  
...
<label><input class="exist-nav" name="investments" type="radio" value="VC-EXIST" />M&A(VC) </label></div>
...
$('.investment-form select, .exit-form select').switchify();
	...
	$('.exitslabel .ui-switch-off').click(function() {   
        $("#exitsviape").show();  
        $("#exitsviavc").hide();
    });	 
	$('.exitslabel .ui-switch-on').click(function() {   
        $("#exitsviape").hide();  
        $("#exitsviavc").show();
    });	
