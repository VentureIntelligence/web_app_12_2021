			...
			//	     echo "<br>all records" .$companysql;
                            ...
                            if($type != 4)
                            {
                                ...
                                if($type==1)
                                {
                                    if($advisorsearchstring_legal!="" || $advisorsearchstring_trans!="")
                                    {
                                        ...
                                        $companysql = "select year(pe.dates),count(distinct pe.PEId), COUNT( DISTINCT CASE WHEN pe.amount >0 AND pe.hideamount =0 THEN pe.PEId END), sum(pe.amount) from peinvestments as pe, industry as i,pecompanies as pec,stage as s ".$keybef." where";
                                    }
                                }
                                else if($type==2)
                                {
				...
				//echo "<br> individual where clauses have to be merged ";
					if ($industry > 0)
						...
						$companysql2 = $companysql2 ." pe.Deleted=0 AND pe.AggHide =0 and pe.amount!=0 AND and pec.industry = i.industryid and pec.PEcompanyID = pe.PECompanyID and pe.StageId=s.StageId " . $addVCFlagqry . "  AND ".$wheredates ." and ";
