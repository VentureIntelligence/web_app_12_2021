		...
		<div class="block">
            <ul > 
                ...
                <li class="tagpadding">
                    ...
                    <input type="button" name="fliter_stage" class="fliter_stage"  value="Filter" style="float: right;">
                </li>
            </ul>
	    </div>
    </div>
    ...
    <div class="block">
        <ul>
            ...
            <li class="odd"><h4>Stage <span><a href="#popup2" class="help-icon1 tooltip"><img width="18" height="18" border="0" src="images/help.png" alt="" style="vertical-align:middle">
                <span><img class="callout1" src="images/callout.gif">Definitions</span></a></span></h4>
            ...
            <li class="odd"><h4>Company Type</h4>
                ...
                <SELECT NAME="comptype" id="comptype" style=" background: <?php echo $background; ?>;" <?php if($disable_flag == "1"){ echo "disabled"; } ?>>
...
<input type="text " name="invrangestart" id="invrangestart" maxlength="5" value="<?php echo $startRangeValue;?>" style=" float:left;width:41%; background: <?php echo $background; ?>;" <?php if($disable_flag == "1"){ echo "disabled"; } ?> onkeypress="return isNumberKey1(event)" autocomplete="off" size="11">
    
<span class="range-text"> to</span>
<input type="text " name="invrangeend" id="invrangeend" maxlength="5" value="<?php echo $endRangeValue;?>" onblur="isless();" style="float:left;width:41%; background: <?php echo $background; ?>;" <?php if($disable_flag == "1"){ echo "disabled"; } ?> onkeypress="return isNumberKey1(event)" autocomplete="off" size="11">
...
function disableFileds(){
 ...
 $('#sltindustry, #stage, #round, #comptype, #dealtype_debtequity, #Syndication,#citysearch,#invType,#invhead,#invrangestart,#invrangeend,#exitstatus,#valuations,#sltstate').css('background-color', "#dddddd !important");
    $(' #comptype, #dealtype_debtequity, #Syndication,#invType,#invhead,#invrangestart,#invrangeend').attr('style', 'background-color: #dddddd !important');
...
function enableFileds(){ 
    ...
    $('#sltindustry, #stage, #round, #comptype, #dealtype_debtequity, #Syndication,#citysearch,#invType,#invhead,#invrangestart,#invrangeend,#exitstatus,#valuations,#sltstate').css('background-color', "#ffffff !important");
    $('#comptype, #dealtype_debtequity, #Syndication,#invType,#invhead,#invrangestart,#invrangeend').attr('style', 'background-color: #ffffff !important');
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if(datacount>0) {
                        var multiselect='';
                        var citySelected='';
                        //multiselect+='<option value="--" id="all_city_selection" >All City</option>';
                        $.each(innerdata, function (key, item) {
                            citySelected+=item['id']+',';
                            multiselect+="<OPTION id='"+item['id']+ "' value="+item['id']+" >"+item['value']+"</OPTION>";
                        });
