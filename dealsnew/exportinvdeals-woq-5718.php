...
if (($keyword == "") && ($searchallfield == "") && ($companysearch == "") && ($advisorsearch_trans == "") && ($advisorsearch_legal == "") && ($regionId == "--" || $regionId =='') && ($industry == "--") && ($companyType == "") && ($invType == "--") && ($startRangeValue == "--") && ($endRangeValue == "--") && ($hidedateStartValue == "------01") && ($hidedateEndValue == "------01") && ($stage == "--") && ($debt_equity == "--")) {
    $companysql = "SELECT PEId,PEId,PEId,pe.PECompanyId, pe.StageId,pec.countryid,
                    ...
                    Revenue_Multiple,EBITDA_Multiple,PAT_Multiple,listing_status,Exit_Status,SPV,AggHide,Revenue,EBITDA,PAT, price_to_book, book_value_per_share, price_per_share,pe.Amount_INR
...
//print_r($_SESSION['pe_inv']);die;
    ...
    $companysql = "SELECT PEId,PEId,PEId,pe.PECompanyId,pe.StageId,pec.countryid,pec.industry,
                    ...
                    pe.Company_Valuation,Revenue_Multiple,EBITDA_Multiple,PAT_Multiple,listing_status,Exit_Status,SPV,AggHide,Revenue,EBITDA,PAT, price_to_book, book_value_per_share, price_per_share,pe.Amount_INR
...
//                    order by  companyname";
    ...
    $companysql = "select peinv.PEId,peinv.PECompanyId,peinv.StageId,pec.countryid,pec.industry,
                   ...
                   Revenue_Multiple,EBITDA_Multiple,PAT_Multiple,listing_status,Exit_Status,SPV,AggHide,Revenue,EBITDA,PAT, price_to_book, book_value_per_share, price_per_share,peinv.Amount_INR
    ...
    $companysql = "(SELECT peinv.PEId,peinv.PECompanyId,peinv.StageId,pec.countryid,pec.industry,cia.CIAId,adac.CIAId AS AcqCIAId,
                    ...
                    pe.Company_Valuation,Revenue_Multiple,EBITDA_Multiple,PAT_Multiple,listing_status,Exit_Status,SPV,AggHide,Revenue,EBITDA,PAT, price_to_book, book_value_per_share, price_per_share,peinv.Amount_INR
...
//exit;
//execute query
$result = @mysql_query($sql) or die("Error in connection:<br>".  mysql_error());
...
while ($row = mysql_fetch_row($result)) {
    ...
    /*$companiessql = "pe.PEId,
        ...
        pe.Deleted=0  and pec.industry !=15 and pe.PEId=".$rows['PEId']."
        AND pe.PEId NOT
        IN (
        SELECT PEId
        FROM peinvestments_dbtypes AS db
