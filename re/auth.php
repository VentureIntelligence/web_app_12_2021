...
if ($_POST['btnSubmit']=='Authorize'){
    ...
    if ($displayMessage==''){
        ...
        $sqlAuthCheck = "SELECT `expDate` FROM `user_auth_code` WHERE `emailId`='".$userEmail."' AND `dbType`='RE' AND `authCode`='$authCode' AND `expDate` >= '".$today."'";
        ...
        if($_POST['autdevice']){
         ...
         $currMACookieArray=  json_decode($currMACookie,true);
          ...
          if($_POST['autdevice'] > 0 && $_POST['autdevice']!=''){
            ...
            $sqlnewdevice = "update user_authorized_device set `status`=1 where id=".$_POST['autdevice'];
            mysql_query($sqlnewdevice); 
            $sqlreuserstatus = "update dealmembers set user_authorization_status=1 where EmailId ='".$userEmail."'";
        ...
        if ($cntAuthCheck==1){
            ...
            if ($devicesUsed <= $allowedNoDevices){
                ...
                if ($devicesUsed <= $allowedNoDevices){
                    ...
                    //Store Cookie value in DB
                    $sqlInsCookie = "INSERT INTO `userlog_device` (`EmailId`,`deviceId`,`DCompId`,`dbType`)";
                    $sqlInsCookie .= " VALUES ('".$userEmail."','".$cookieName."','".$cntgetdevRes['DCompId']."','MA')";
                    mysql_query($sqlInsCookie) or die(mysql_error());
                ...
                if ($devicesUsed < $allowedNoDevices){
                    ...
                    //Store Cookie value in DB
                    $sqlInsCookie = "INSERT INTO `userlog_device` (`EmailId`,`deviceId`,`DCompId`,`dbType`)";
                    $sqlInsCookie .= " VALUES ('".$userEmail."','".$cookieName."','".$cntgetdevRes['DCompId']."','PE')";
                    mysql_query($sqlInsCookie) or die(mysql_error());
                ...
                //Message to login
                /*$successMessage = 'You have successfully authorized this device. <a href="relogin.php">Login Here</a>';
                $userEmail = '';
                $authCode  = '';*/
                
//                header("location:relogin.php?auth=1");
                ...
                mysql_query($sqlmauserstatus) or die(mysql_error());
                 ...
                 header( 'Location: '. GLOBAL_BASE_URL .'/relogin.php?auth=1' ) ;
                exit;
...
<?php  /* <li><label>Email</label> <input type=text name="emailid" value="<?php echo $userEmail; ?>" size="23" style="padding: 5px;"/></li> */ ?>
