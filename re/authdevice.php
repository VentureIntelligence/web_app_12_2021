                ...
                While($myrow = mysql_fetch_array($totalrs))
                {
                    ...
                    if( date('Y-m-d')<=$myrow["ExpiryDate"]){
                        if($myrow['EmailId'] != ''){
                            ...
                            header("Location:".BASE_URL."re/auth.php?device=".$_POST['dauth']."&email=".$_POST['user_email']."&device_detail=".$_POST['device_details']); 
                            // header("Location: ".BASE_URL."dealsnew/auth.php?device=".$_POST['dauth']."&email=".$_POST['user_email']);
                            die();
                        }else{
                            header("Location:".BASE_URL."relogin.php?emailValidation=0");
                            die();
                        }
                    } 
                    else{
...
function sendAuthEmail($companyId,$userEmail,$allowedDevices){
    ...
    //$message 	.= "<p><b>Authorization Link:</b> <a href='http://www.ventureintelligence.asia/dev/re/auth.php?device=".$_POST['dauth']."'>http://www.ventureintelligence.asia/dev/re/auth.php?device=".$_POST['dauth']."</a></p>";
    //$message 	.= "<p><b>Authorization Link:</b> <a href='http://www.ventureintelligence.asia/dev/re/auth.php?device=".$_POST['dauth']."&email=".$userEmail."'>http://www.ventureintelligence.asia/dev/re/auth.php?device=".$_POST['dauth']."</a></p>";
    $message 	.= "<p><b>Authorization Link:</b> <a href='".BASE_URL."re/auth.php?device=".$_POST['dauth']."&email=".$_POST['user_email']."&device_detail=".$_POST['device_details']."'>".BASE_URL."dealsnew/auth.php?device=".$_POST['dauth']."</a></p>";
