...
//			 AND  pec.PECompanyId=$SelCompRef";
        ...
        $sql="SELECT pe.FinLink, pe.uploadfilename, pec.PECompanyId, pec.companyname, pec.industry, i.industry, pec.sector_business, website,linkedIn,
                        stockcode, yearfounded, pec.Address1, pec.Address2, pec.AdCity, pec.Zip, pec.OtherLocation, 
...
//			
//            if($pe_re==0)  //investors in RE investments
//		{
//			$investorSql="select pe.PEId as DealId,peinv.PEId,peinv.InvestorId,inv.Investor,DATE_FORMAT( dates, '%b-%Y' ) as dt from
//			REinvestments as pe, REinvestments_investors as peinv,REcompanies as pec,
//			REinvestors as inv where pe.PECompanyId=$SelCompRef and
...
//			//echo "<br>^^".$investorSql;
//		}
//		if($pe_re==1)  //investors in RE-IPO Exits
//		{
...
//			and pec.PEcompanyId=pe.PECompanyId  ORDER BY IPODate  DESC";
//
//			$dealpage="reipodealdetails.php";
//			//echo "<br>^**^".$investorSql;
//		}
//		elseif($pe_re==2) // investors in RE-M&A Exits
//		{
...
//				WHERE pe.PECompanyId =$SelCompRef
//				AND peinv.MandAId = pe.MandAId
//				AND inv.InvestorId = peinv.InvestorId
//				AND pec.PEcompanyId = pe.PECompanyId   ORDER BY DealDate  DESC";
//
...
//		}
		...
		where pec.PECompanyId=$SelCompRef and mgmt.PECompanyId=pec.PECompanyId and exe.ExecutiveId=mgmt.ExecutiveId";
			...
			DealAmount, DATE_FORMAT( DealDate, '%b-%Y' ) as dt, pe.MandAId, rei.InvestorId, rei.MandAId, inv.InvestorId, inv.Investor
			...
			AND pec.PEcompanyId = pe.PECompanyId and pe.Deleted=0 and pec.industry IN (2,3,4,5,15) and pe.PECompanyId=$SelCompRef order by DealDate desc";
                 
                 
                 $ipoexitsql="SELECT pe.PECompanyId, pec.companyname, pec.industry, i.industry, pec.sector_business,
				IPOAmount, DATE_FORMAT( IPODate, '%b-%Y' ) as dt, pe.IPOId, rei.InvestorId, rei.IPOId, inv.InvestorId, inv.Investor
				FROM REipos AS pe, reindustry AS i, REcompanies AS pec, REipo_investors AS rei, REinvestors AS inv WHERE rei.InvestorId=inv.InvestorId AND  pe.IPOId=rei.IPOId AND i.industryid=pec.industry
			AND pec.PEcompanyId = pe.PECompanyId and pe.Deleted=0 and pec.industry IN (2,3,4,5,15) and pe.PECompanyId=$SelCompRef order by IPODate desc";
        ...
        {
            $listviewurl="reipoindex.php?value=1";
...
<div class="result-cnt">
    ...
    <div class="list-tab"><ul>
        ...
        <li class="active"><a id="icon-detailed-view" class="postlink" href="recompanydetails.php?value=<?php echo $_GET['value'];?>" ><i></i> Detail  View</a></li> 
        </ul></div> 
...
<div class="view-detailed"> 
    ...
    <div class="detailed-title-links">
		...
		<?php if ($prevKey!='-1') {?> <a  class="postlink" id="previous" href="recompanydetails.php?value=<?php echo $prevNextArr[$prevKey]."/".$pe_re; ?> ">< Previous</a><?php } ?> 
            <?php if ($nextKey < count($prevNextArr)) { ?><a class="postlink" id="next" href="recompanydetails.php?value=<?php echo $prevNextArr[$nextKey]."/".$pe_re; ?> "> Next > </a>  <?php } ?>
                    </div> 
...
?>                                                  
    ...
    <?php
        ...
        While($myInvestorrow=mysql_fetch_array($getcompanyrs, MYSQL_BOTH))
        {
            ...
            {
            ?>
            <tr><td style="alt"><a class="postlink" href='reinvestordetails.php?value=<?php echo $myInvestorrow["InvestorId"];?>/<?php echo $pe_re;  ?>' title="Investor Details"><?php echo $myInvestorrow["Investor"]; ?></a></td>
                <td><a href=<?php echo $dealpage;?>?value=<?php echo $myInvestorrow["DealId"];?>/<?php echo $pe_re;  ?> title="Deal Details"><?php echo $myInvestorrow["dt"];?> </a></td></tr>                                                          
            <?php 
            }
            else {
                 ...
                 <?php
                        ...
                        While($ipoexitrow=mysql_fetch_array($rsipoexit, MYSQL_BOTH))
                        {
                            ...
                            <tr><td style="alt">IPO</td>
                              <td><?php echo $ipoexitrow["Investor"];?></td>
                            <td> <a href='reipodealdetails.php?value=<?php echo $ipoexitrow["IPOId"];?>' title="Deal Details"><?php echo $ipoexitrow["dt"];?></a></td>
                            </tr>
                            <?php
                        }
                    ...
                    if($rsmandaexit= mysql_query($maexitsql))
                        ...
                        <td> <a href='remandadealdetails.php?value=<?php echo $mymandaexitrow["MandAId"];?>' title="Deal Details"><?php echo $mymandaexitrow["dt"];?></a></td>
...
if($myrow["uploadfilename"]!="")
{
     ...
     <form name="recompanyDisplay"  id="companyDisplay" method="post" action="exportrecompanyprofile.php">
