...
//                }
        ...
        else 
        {
            ...
            if($advisorsearchstring_legal!=''){
                ...
                $advisorsql= "select cianame from REadvisor_cias where CIAId='$advisorsearchstring_legal_id'";
        ...
        else 
        {
            ...
            if($advisorsearchstring_trans!=''){
                ...
                $advisorsql= "select cianame from REadvisor_cias where CIAId='$advisorsearchstring_trans_id'";
       ...
       $stageCnt=0;
		...
		elseif($companyType=="U")
                        ...
                        elseif ($searchallfield != "")
			{
				...
				WHERE pe.dates between '" . $dt1. "' and '" . $dt2 . "' and pe.IndustryId = i.industryid AND pec.PEcompanyID = pe.PECompanyID and pe.StageId=s.RETypeId and pe.Deleted =0 AND REinvoinv.PEId = pe.PEId AND REinv.InvestorId = REinvoinv.InvestorId AND ( pec.city LIKE '$searchallfield%' or pec.companyname LIKE '$searchallfield%'
				OR sector_business LIKE '$searchallfield%' or MoreInfor LIKE '%$searchallfield%' or REinv.Investor like '$searchallfield%' or pe.ProjectName like '$searchallfield%' or (reacai.Cianame like '$searchallfield%'  and  reacai.dates between '" . $dt1. "' and '" . $dt2 . "'  AND reacai.PEId=REinvoinv.PEId)    )  
                                GROUP BY pe.PEId ";
                                 $orderby="dates";
                            $ordertype="desc";
...
//                                                $sector_sql[] = " sector_business LIKE '$word%' ";
			...
			$companysql="(
				...
				FROM REinvestments AS peinv, REcompanies AS c, reindustry AS i, REadvisor_cias AS cia, REinvestments_advisorinvestors AS adac
                                WHERE dates between '" . $dt1. "' and '" . $dt2 . "' and  peinv.Deleted=0 and peinv.IndustryId = i.industryid AND c.PECompanyId = peinv.PECompanyId
			...
			elseif (($industry > 0) || ($invType !="--") || ($companyType!="--") || ($regionId> 0) || ($city != "") ||  ($entityProject!="--") || ($startRangeValue == "--") || ($endRangeValue == "--") || ( ($exitstatusValue!='' && $exitstatusValue!='--')) || (($month1 != "--") && ($year1 != "--")  && ($month2 !="--") && ($year2 != "--")) .$checkForStageValue)
				{
				    $yourquery=1;
                      ...
                      <li> <a id="icon-detailed-view" class="tour-lock postlink" href="redealdetails.php?value=<?php echo $comid;?>" onmouseover="blocktourover();" onmouseout="blocktourout();"><i></i> Detail View</a></li> 
                        ...
                        <div class="view-table view-table-list">
                            <table width="100%" border="0" cellspacing="0" cellpadding="0" id="myTable">
                              <thead><tr>
                                <th style="width: 782px;" class="header <?php echo ($orderby=="companyname")?$ordertype:""; ?>" id="companyname">Company</th>
                                <th style="width: 195px;" class="header <?php echo ($orderby=="dealcity")?$ordertype:""; ?>" id="dealcity">City</th>
                                <th style="width: 414px;" class="header <?php echo ($orderby=="investor")?$ordertype:""; ?>" id="investor">Investor</th>
                                <th style="width: 170px;" class="header <?php echo ($orderby=="dates")?$ordertype:""; ?>" id="dates">Date</th>
                                 <th style="width: 200px;" class="header <?php echo ($orderby=="Exit_Status")?$ordertype:""; ?>" id="Exit_Status">Exit Status</th>
                                <th style="width: 140px;" class="header asc <?php echo ($orderby=="amount")?$ordertype:""; ?>" id="amount">Amount</th>
                                </tr></thead>
                              <tbody id="movies">
                                <?php
		...
		else
		{
					...
					$('#exportbtn').html('<input class ="export" type="button" id="expshowdeals"  value="Export" name="showdeals">');
                            ...
                            ?>
                                    ...
                                    <a class ="export" target="_blank" href="../xls/Sample_Sheet_Investments-RE.xls">Sample Export</a>
                                    </span>
    ...
    function initExport(){ 
            ...
            $.ajax({
                ...
                $(document).on('click', 'tr .details_link', function(){ 
                    ...
                    $("#detailpost").attr("href","<?php echo GLOBAL_BASE_URL; ?>re/redealdetails.php?value="+idval).trigger("click");
                });
            </script>
    ...
    $(document).ready(function(){
     ...
     <?php } ?>
       ...
       function searchcloseout()
       {
        ...
        <?php if($tourautostart!='ON' && $_SESSION['re_popup_display'] != 'none'){  ?>
            <script>
			   $(document).ready(function(){
